...
def crop_image(image, boxes, crop_size):
    """
    Crop the image around each bounding box and resize to a standard size.
    """
    ...
    for box in boxes:
        xc, yc, w, h = box
        x_min = max(0, int((xc - w / 2) * image_width))
        y_min = max(0, int((yc - h / 2) * image_height))
        x_max = min(image_width, int((xc + w / 2) * image_width))
        y_max = min(image_height, int((yc + h / 2) * image_height))

        crop = image.crop((x_min, y_min, x_max, y_max))
        crop = crop.resize((crop_size, crop_size), Image.ANTIALIAS)
        crops.append(crop)
