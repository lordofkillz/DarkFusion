...
from PyQt5.QtCore import (QCoreApplication, QDir, QEvent, QModelIndex, QObject,
                          ...
                          QTimer, QUrl, pyqtSignal, pyqtSlot, QPointF,QModelIndex,Qt,QEvent,QByteArray, QBuffer, QIODevice)
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def delete_thumbnail(self, image_file, bbox_index):
        ...
        thumbnail_filename = f"{base_filename}_{bbox_index}.jpeg"
        ...
        if os.path.exists(thumbnail_path):
            ...
            print(f"No thumbnail found for {thumbnail_path}")

        # Handle any associated QGraphicsItem instances in the scene
   ...
   # Ensure compare_images and related hashing functions are also part of the class
    ...
    def pixmap_hash(self, pixmap):
        """Generate a hash for a QPixmap object."""
        ...
        image.save(buffer_stream, 'PNG')
        # Use hashlib to generate a hash of the image data
        hash_val = hashlib.sha256(buffer.data()).hexdigest()
        return hash_val           
    def extract_and_display_data(self, new_size):
            ...
            # Filter image and text files from all files
            images_files = [f for f in all_files if f.endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif'))]
            txt_files = [f for f in all_files if f.endswith('.txt')]

            # Warn and exit if no images are found
            if not images_files:
            ...
            self.preview_list.setHorizontalHeaderLabels(['Image', 'Class Name', 'ID', 'Size', 'Bounding Box'])
            ...
            if os.path.exists(classes_file_path):
                with open(classes_file_path, 'r') as f:
                    for class_id, line in enumerate(f.readlines(), start=1):  # Adjust class ID index based on your data setup
                        class_name = line.strip()
                        self.id_to_class[class_id - 1] = class_name  # Mapping class IDs to class names
            ...
            self.preview_list.setColumnWidth(1, 200)
            self.preview_list.setColumnWidth(2, 50)
            self.preview_list.setColumnWidth(3, 100)
            self.preview_list.setColumnWidth(4, 100)
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                for image_file in images_files:
                    ...
                    pixmap = self.load_image(image_file)
                    ...
                    for i, line in enumerate(lines):
                        ...
                        thumbnail_filename = os.path.join(
                            self.thumbnails_directory, f"{base_file_name}_{i}.jpeg")  # Fixed to use jpeg and self.thumbnails_directory
                        ...
                        # Resize columns and rows in the preview list to fit the new content
    ...
    def load_image(self, image_file):
        try:
            ...
            logging.error(f"Error reading image file {image_file}: {str(e)}")
            return None
    ...
    def move_filtered_images(self, filter_input):
        ...
        for file_path in self.filtered_image_files:
            if self.is_file_matching_class_id(file_path, filter_id):
    ...
    def filter_class(self, filter_input):
        ...
        # Determine if the input is a class ID (numeric) or a class name (non-numeric)
        try:
        ...
        for img_file in self.image_files:
            ...
            if os.path.isfile(label_file):
                with open(label_file, 'r') as f:
                    ...
                    if self.filter_blanks_checkbox.isChecked() and content == '':
...
# adjust and view labels for a single image
    ...
    def create_mosaic_images(self):
        ...
        while thumbnail_paths and (max_images is None or image_count < max_images):
            ...
            output_image_path = os.path.join(output_dir, f"mosaic_{image_count:04d}.jpeg")
