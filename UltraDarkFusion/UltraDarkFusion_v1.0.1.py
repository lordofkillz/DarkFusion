...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def auto_label_images2(self):
        ...
        for idx, image_file in enumerate(self.image_files):
            ...
            try:
                if self.model_type == 'yolov8': # Adjust as needed for half precision or model types         
            ...
            try:
                results = self.model(image_file)  # Run inference on the image

                # Handle pose estimation
                if 'pose' in self.weights_file_path:
                    keypoints = results.xyxy[0]  # Extract keypoints if the model is for pose estimation
                    with open(label_file, 'w') as f:
                        for keypoint in keypoints:
                            # Writing class, x, y, and confidence for each keypoint
                            f.write(f"{keypoint[5]} {keypoint[0]} {keypoint[1]} {keypoint[4]}\n")

                # Handle segmentation
                elif 'seg' in self.weights_file_path:
                    segmentation_masks = results.pred[0]  # Extract segmentation masks
                    # Additional code needed to process and write segmentation data
...
# for blanks
    ...
    def open_image_video(self):
        ...
        if dir_name:
            ...
            # Update the classes dropdown after setting the clasdddddddddddds names
...
# adjust and view labels for a single image
    ...
    def ultralytics_train_clicked(self):
        ...
        command = f"yolo detect train data={data_input} imgsz={imgsz_input} epochs={epochs_input} " \
                f"batch={batch_input} {model_arg} project={self.runs_directory}  patience=10"
    ...
    def handle_conversion(self):
        ...
        # Get parameters based on the checkbox states
        imgsz = self.imgsz_input_2.text()
        ...
        use_gpu = self.use_gpu_checkbox.isChecked()  # Replace with your actual checkbox

        # Convert imgsz to a list containing an integer
        try:
            imgsz_list = [int(imgsz)]  # Convert to an integer and wrap in a list
        except ValueError:
            QMessageBox.critical(self, "Conversion Error", f"Invalid image size value: {imgsz}")
            return
