...
# for blanks
        ...
        if new_index != -1:
            ...
            if self.heads_area.isChecked() and new_index != 1:
                ...
                try:
                    ...
                    print(f"Warning: Invalid class ID '{self.class_id.text()}' provided in QLineEdit.")
                    # Append the next available class ID in the list
    ...
    def get_next_available_class_id(self):
        # Assuming the dropdown has class IDs in order, find the next available class ID
        current_index = self.classes_dropdown.currentIndex()
        next_index = (current_index + 1) % self.classes_dropdown.count()
        return next_index
