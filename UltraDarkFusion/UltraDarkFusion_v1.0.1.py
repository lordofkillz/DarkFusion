...
class CustomGraphicsView(QGraphicsView):
    ...
    def _setup_initial_state(self):
        ...
        self.main_window.crosshair_color.triggered.connect(self.pick_color)
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        # Find the 'Reset Layout' action by its name
        ...
        self.continuous_delete_timer = QTimer(self)
        self.continuous_delete_timer.setInterval(100)  # Adjust interval as needed
        self.continuous_delete_timer.timeout.connect(self.delete_current_item)
        self.current_item_index = None  # Initialize the variable to track the current item
         # global event filter to catch all events
    ...
    def update_roi(self):
        if self.roi_checkbox.isChecked():
            ...
            # The offsets for x and y will be half the width and height respectively
    ...
    def predict_and_draw_yolo_objects(self, image, image_file_path):
        ...
        for yolo_line in yolo_lines:
            ...
            try:
                ...
                # Enable autocast for the prediction block
                with autocast():
...
# for blanks
    ...
    def eventFilter(self, source, event):
        ...
        if event.type() == QEvent.MouseButtonPress and source is self.preview_list.viewport():
            ...
            if item and event.button() == Qt.RightButton:
                # Check if the rapid_checkbox is checked before initiating continuous deletion
                if self.rapid_checkbox.isChecked():
                    self.current_item_index = self.preview_list.indexFromItem(item)
                    self.continuous_delete_timer.start()
                else:
                    # If the checkbox is not checked, handle a single right-click deletion
                    self.handle_right_click(self.preview_list.indexFromItem(item))
            elif item and event.button() == Qt.LeftButton:
                self.handle_left_click(self.preview_list.indexFromItem(item))

        # Catching Mouse Released Event
        elif event.type() == QEvent.MouseButtonRelease and event.button() == Qt.RightButton:
            # Stop the continuous delete timer if it was started
            if self.rapid_checkbox.isChecked():
                self.continuous_delete_timer.stop()
                self.current_item_index = None  # Invalidate the current index

        # Handling the event normally after filtering
    ...
    def delete_current_item(self):
        if self.current_item_index is not None and self.current_item_index.isValid():
            # Perform deletion logic here using self.current_item_index
            self.handle_right_click(self.current_item_index)

            # Check if there are any items left to delete after the current one is removed
            remaining_items = self.preview_list.model().rowCount()
            next_row = self.current_item_index.row()  # The next row after deletion

            if next_row < remaining_items:
                # There are still items to delete
                self.current_item_index = self.preview_list.model().index(next_row, 0)
            else:
                # No more items to delete, stop the timer
    ...
    def pixmap_hash(pixmap):
        """Calculate the hash of a QPixmap."""
        buffer = QBuffer()
        buffer.open(QIODevice.WriteOnly)
        pixmap.save(buffer, 'PNG')  # Saving QPixmap to a buffer in PNG format
        pixmap_data = buffer.data()
        return hashlib.sha256(pixmap_data).hexdigest()
    ...
    def save_progress(self, progress_file, progress_data):
    ...
    def extract_and_display_data(self, new_size):
        ...
        progress_file = os.path.join(data_directory, "progress.json")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            for image_file in images_files:
                ...
                for i, line in enumerate(lines):
                    ...
                    progress_data[base_file_name] = True  # Mark as processed
...
# adjust and view labels for a single image
    ...
    def apply_stylesheet(self):
        ...
        if selected_style == "Default":
            ...
            apply_stylesheet(self.app, selected_style)  # Assuming this function is correctly defined
        else:
            style_folder = QDir.currentPath() + '/styles'
            ...
            try:
                ...
                # You might want to do additional error handling here
