...
class CustomGraphicsView(QGraphicsView):
    ...
    def _handle_left_button_press(self, event):
        ...
        elif event.type() == QEvent.MouseButtonDblClick: # type: ignore
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.id_to_class = {}
        self.image_size.setMinimum(1)
        self.image_size.setMaximum(self.MAX_SIZE)
        self.image_size.setValue(150)
        self.thumbnail_memory_cache = {}
        self.thumbnails_directory = None
        QApplication.instance().installEventFilter(self)
        self.signalThumbnailCreated.connect(self.update_ui_with_thumbnail)
        ...
        self.img_video_button.triggered.connect(self.enable_image_loading_mode)
    ...
    def predict_and_draw_yolo_objects(self, image, image_file_path):
        ...
        for index, adjusted_box in enumerate(adjusted_boxes):
            ...
            if np.count_nonzero(segmented_thumbnail) > (segmented_thumbnail.size // 10):  # More than 10% non-black
                thumbnail_filename = f"seg{self.thumbnail_index:04d}.png"
        ...
        for idx, image_file in enumerate(self.image_files):
            ...
            try:
                ...
                if self.model_type in ['yolov8', 'yolov8_trt', 'onnx']:
                    ...
                    results = self.model(image_file, **model_kwargs) # type: ignore
    ...
    def _perform_size_adjustment(self):
        # Assuming self._image_size_value is the new desired size for thumbnails
        for row in range(self.preview_list.rowCount()):
            thumbnail_label = self.preview_list.cellWidget(row, 0)  # Adjust the index as per your setup
            if thumbnail_label:  # If there's a thumbnail
                # Get the existing QPixmap, resize it, and set it back
                pixmap = thumbnail_label.pixmap()
                resized_pixmap = pixmap.scaled(self._image_size_value, self._image_size_value, Qt.KeepAspectRatio, Qt.SmoothTransformation)
                thumbnail_label.setPixmap(resized_pixmap)
        
        # Optionally, adjust column width to match new thumbnail size
        self.preview_list.setColumnWidth(0, self._image_size_value)
    ...
    def delete_item(self, row, image_item, label_item):
        ...
        self.update_label_file(image_file, label_to_delete)
    ...
    def delete_thumbnail(self, image_file, bbox_index):
        thumbnail_filename = f"{os.path.splitext(os.path.basename(image_file))[0]}_{bbox_index}.png"
        thumbnail_path = os.path.join(self.thumbnails_directory, thumbnail_filename)
        if os.path.exists(thumbnail_path):
            os.remove(thumbnail_path)
            print(f"Deleted thumbnail: {thumbnail_path}")
        
        # Assuming the bounding boxes are QGraphicsItem instances in screen_view's scene
        # and you have a way to correlate them with image_file and bbox_index.
        for item in self.screen_view.scene().items():
            if hasattr(item, 'unique_id') and item.unique_id == f"{image_file}_{bbox_index}":
                self.screen_view.scene().removeItem(item)
                break
        self.screen_view.update()  # Refresh the QGraphicsView

    def realign_remaining_entries(self):
        for i in range(self.preview_list.rowCount()):
            bbox_item = self.preview_list.item(i, 5)
            if bbox_item:
    ...
    def load_classes(self):
        ...
        try:
            with open('classes.txt', 'r') as file:
                self.id_to_class = {int(class_id)-1: class_name.strip() for class_id, class_name in enumerate(file, start=1)}
        except Exception as e:
    ...
    def handle_left_click(self, index):
        ...
        # Fetch the image file path from the selected item in preview_list
        item = self.preview_list.itemFromIndex(index)  # Always get a fresh item from index
        image_file = item.text()

        # Display the clicked image in the screen_view.
        self.display_image(image_file)
        ...
        # Ensure labels are updated based on the hide_labels flag.
        if self.hide_labels:  # If labels are currently hidden, show them
            self.hide_labels = False
            self.toggle_label_visibility()

        # Flash the bounding box if there's one related to the clicked thumbnail.
        bounding_box_item = self.preview_list.item(row, 5)
        if bounding_box_item:
            bounding_box_index = bounding_box_item.data(Qt.UserRole)
            # Now let's flash the bounding box.
            for rect_item in self.screen_view.scene().items():
                if isinstance(rect_item, BoundingBoxDrawer) and rect_item.unique_id == f"{image_file}_{bounding_box_index}":
                    rect_item.flash_color = QColor(*self.flash_color_rgb)  # Set flash color
                    rect_item.flash(duration=self.flash_time.value())  # Implement this method as needed
                    break

        # Synchronize the selection in the ListView with the thumbnail.
        self.synchronize_list_view(image_file)

    def synchronize_list_view(self, image_file):
        file_basename = os.path.basename(image_file)
        self.List_view.clearSelection()  # Clear previous selection.
        list_view_model = self.List_view.model()
        for i in range(list_view_model.rowCount()):
            if list_view_model.item(i).text() == file_basename:
                matching_index = list_view_model.index(i, 0)
    ...
    def flash_bounding_box_with_index_and_file(self, bounding_box_index, image_file):
        unique_key = f"{image_file}_{bounding_box_index}"
        bounding_box_details = self.bounding_boxes.get(unique_key)
        if bounding_box_details:
            self.flash_bounding_box_with_duration(unique_key, self.flash_time.value())
    ...
    def save_cropped_images(self, pixmap, x_center, y_center, image_file, i):
        ...
        # Now calculate_crop_coordinates is called correctly as a method of the class
        crop_x1, crop_y1, crop_x2, crop_y2 = self.calculate_crop_coordinates(x_center, y_center, crop_width, crop_height, pixmap)

        cropped_pixmap = pixmap.copy(crop_x1, crop_y1, crop_x2 - crop_x1, crop_y2 - crop_y1)
        if cropped_pixmap.width() != crop_width or cropped_pixmap.height() != crop_height:
            return

        cropped_directory = os.path.join(self.image_directory, "cropped")
        os.makedirs(cropped_directory, exist_ok=True)
        base_file_name, ext = os.path.splitext(os.path.basename(image_file))
        cropped_filename = os.path.join(cropped_directory, f"{base_file_name}_{i}_cropped{ext}")

        # Your existing logic to compare and save the cropped image
        if not os.path.exists(cropped_filename) or not self.compare_images(cropped_pixmap, cropped_filename):
            cropped_pixmap.save(cropped_filename)

    def calculate_crop_coordinates(self, x_center, y_center, crop_width, crop_height, pixmap):
        """Calculate cropping coordinates centered around (x_center, y_center)."""
   ...
   # Ensure compare_images and related hashing functions are also part of the class
    def compare_images(self, new_pixmap, existing_file_path):
        """Compare 'new_pixmap' with the image at 'existing_file_path'."""
        new_pixmap_hash = self.pixmap_hash(new_pixmap)
        existing_image_hash = self.image_hash(existing_file_path)
        return new_pixmap_hash == existing_image_hash


    def initialize_cache(self):
        thumbnails_dir = os.path.join(self.image_directory, "thumbnails")
        os.makedirs(thumbnails_dir, exist_ok=True)
        
        # Load the existing thumbnails into the cache
        for thumbnail_file in os.listdir(thumbnails_dir):
            self.thumbnail_memory_cache[thumbnail_file] = True
    ...
    def align_progress_with_thumbnails(self, progress_file, thumbnails_dir):
        progress_data = self.load_progress_modified(progress_file)
        thumbnails = set(os.listdir(thumbnails_dir))

        # Regenerate missing thumbnails or mark as unprocessed
        for image_file, processed in progress_data.items():
            image_hash = hashlib.md5(image_file.encode()).hexdigest()
            thumbnail_file = f"{image_hash}.png"

            if processed and thumbnail_file not in thumbnails:
                # Option 1: Regenerate thumbnail
                self.create_or_load_thumbnail(image_file)

        # Save any changes made to progress data
        for filename, processed in progress_data.items():
            # Assuming filenames are the keys in your cache and the processed status is the value
            self.thumbnail_memory_cache[filename] = processed


        # Optional: Handle orphaned thumbnails
        for thumbnail_file in thumbnails:
            if thumbnail_file.replace('.png', '') not in [hashlib.md5(img_file.encode()).hexdigest() for img_file in progress_data.keys()]:
                os.remove(os.path.join(thumbnails_dir, thumbnail_file))  # Delete orphaned thumbnail

        print("Progress and thumbnails aligned.")


    def create_or_load_thumbnail(self, image_file):
        image_path = os.path.join(self.image_directory, image_file)
        image_hash = self.image_hash(image_path) + ".png"
        thumbnail_path = os.path.join(self.image_directory, "thumbnails", image_hash)
        
        # Check if the thumbnail exists in cache or on disk
        if os.path.exists(thumbnail_path):
            # Load from disk if not in cache
            if image_hash not in self.thumbnail_memory_cache:
                self.thumbnail_memory_cache[image_hash] = True
    ...
    def update_ui_with_thumbnail(self, row, thumbnail):
        # Perform any necessary checks or preparation here.
        # Then invoke the actual UI update method in a thread-safe manner.
        QMetaObject.invokeMethod(self, "_thread_safe_update_ui_with_thumbnail", Qt.QueuedConnection,
                                 Q_ARG(int, row), Q_ARG(QPixmap, thumbnail))

    # This is the internal slot that should only be called in a thread-safe manner,
    # typically through invokeMethod to ensure it runs on the main GUI thread.
    @pyqtSlot(int, QPixmap)
    def _thread_safe_update_ui_with_thumbnail(self, row, thumbnail):
        # Now update the UI with the provided thumbnail.
        thumbnail_label = QLabel()
        thumbnail_label.setPixmap(thumbnail)
        # Assuming 'self.preview_list' is a QTableWidget or similar.
        self.preview_list.setCellWidget(row, 0, thumbnail_label)  # row, column
        # ... additional UI update logic can go here.

    def process_images_in_batches(self, image_files, batch_size=100):
        # Split the list of image files into batches
        for i in range(0, len(image_files), batch_size):
            batch = image_files[i:i+batch_size]
            self.process_batch2(batch)
            
    def process_batch2(self, batch):
        with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
            # Process each file in the batch
            future_to_image = {executor.submit(self.create_or_load_thumbnail, image_file): image_file for image_file in batch}
            for future in as_completed(future_to_image):
                image_file = future_to_image[future]
                try:
                    thumbnail, row = future.result()
                    # UI update for the batch can be optimized here
                    self.update_ui_with_thumbnail(row, thumbnail)
    ...
    def extract_and_display_data(self, new_size):
        ...
        # Initialize your cache if not already done; this might happen in your application's startup
        self.initialize_cache()

        # Replace the direct loading of progress with a check in your cache
        progress_data = {filename: self.thumbnail_memory_cache.get(filename, False)
                        for filename in os.listdir(self.image_directory)
                        if any(filename.endswith(ext) for ext in ('.png', '.jpg', '.jpeg', '.bmp', '.gif'))}


        all_files = glob.glob(os.path.join(data_directory, '*'))
        images_files = [f for f in all_files if f.endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif'))]

        # Define txt_files here before using it
        txt_files = [f for f in all_files if f.endswith('.txt')]

        if not images_files:
            QMessageBox.warning(self, "No Images Found",
                                "No images found. Please load images before adjusting the slider.")
            return
        ...
        self.preview_list.hideColumn(6)
        ...
        # Skip if already processed
            if progress_data.get(base_file_name, False):
                print(f"Skipping {base_file_name}, already processed.")
                continue
            ...
            for image_file in images_files:
                ...
                for i, line in enumerate(lines):
                    ...
                    thumbnail_filename = os.path.join(
                        self.thumbnails_directory, f"{base_file_name}_{i}.png")  # Fixed to use .png and self.thumbnails_directory
                    ...
                    QApplication.processEvents()
                    # After successful processing of the image:
                    for filename, processed in progress_data.items():
                        # Assuming filenames are the keys in your cache and the processed status is the value
                        self.thumbnail_memory_cache[filename] = processed
    ...
    def loadSettings(self):
        try:
            ...
            settings = {
                'lastTheme': 'Default',  # Use 'Default' to indicate no stylesheet
                'lastWeightsPath': '',
                'lastCfgPath': '',
                'lastPTWeightsPath': '',
                'last_dir': '',
...
# adjust and view labels for a single image
    ...
    def populate_style_combo_box(self):
        ...
        # Call apply_stylesheet here after setting the index
