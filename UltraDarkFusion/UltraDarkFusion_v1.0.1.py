...
from PyQt5.QtGui import (QBrush, QColor, QFont, QImage, QImageReader,
                         ...
                         QStandardItemModel, QTransform, QLinearGradient,QIcon)
from PyQt5.QtWidgets import (QApplication, QFileDialog,
                             QGraphicsDropShadowEffect, QGraphicsItem,
                             QGraphicsPixmapItem, QGraphicsRectItem,
                             QGraphicsScene, QGraphicsTextItem, QGraphicsView,
                             QLabel, QMessageBox, QProgressBar,
                             QTableWidgetItem, QColorDialog, QMenu,QSplashScreen,QTableView, QVBoxLayout,QWidget)
...
class SafeRotatingFileHandler(RotatingFileHandler):
    """
    Override the RotatingFileHandler to safely handle the PermissionError
    encountered during log rollover on Windows.
    """
    def doRollover(self):
        """
        ...
        try:
            ...
            # Here you can log an error message or pass
            pass
...
# Set up file handler for logging using the custom SafeRotatingFileHandler
file_handler = SafeRotatingFileHandler(os.path.join('debug', 'my_app.log'), maxBytes=10000, backupCount=3)
...
# Custom LoggerAdapter to prepend the current date and time to log messages
...
# Your application setup and logging example
app = QApplication([])  # Replace with your actual application setup

# Check for CUDA availability in PyTorch and OpenCV
pytorch_cuda_available = torch.cuda.is_available()  # Replace with your actual PyTorch CUDA check
opencv_cuda_available = cv2.cuda.getCudaEnabledDeviceCount() > 0  # Replace with your actual OpenCV CUDA check
...
class BoundingBoxDrawer(QGraphicsRectItem):
    ...
    def update_class_color_and_position(self):
        ...
        self.class_name_item.setDefaultTextColor(QColor(255, 215, 0))
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def predict_and_draw_yolo_objects(self, image, image_file_path):
        ...
        # Check cache first for file reading
        if yolo_file_path in self.yolo_cache:
            yolo_lines = self.yolo_cache[yolo_file_path]
        else:
            try:
                with open(yolo_file_path, "r") as f:
                    yolo_lines = f.readlines()
                self.yolo_cache[yolo_file_path] = yolo_lines  # Store contents in cache
            except Exception as e:
        ...
        for yolo_line in yolo_lines:
            ...
            try:
                ...
                logging.error(f'Skipping image due to invalid bounding box values: {e}')
                continue  # This skips the rest of the loop body and proceeds with the next iteration
            ...
            cv2.putText(image, "ORIGINAL", (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1, cv2.LINE_AA)
            cv2.putText(image, "PREDICTED", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
            cv2.rectangle(image, (box[0], box[1]), (box[2], box[3]), (0, 255, 255, 0.1), thickness=1, lineType=cv2.LINE_AA)
            ...
            if self.overwrite:
                ...
                s = "%d %.6f %.6f %.6f %.6f \n" % (class_index, t[0], t[1], t[2], t[3])  # Use original class_index
                adjusted_boxes.append(s)
            image = masked_image

        if self.overwrite:
            # Specify a buffer size; this is 8KB, but you can adjust as needed
            buffer_size = 8 * 1024

            with open(yolo_file_path, "wb", buffering=0) as f:
                with io.BufferedWriter(f, buffer_size=buffer_size) as bf:
                    # Since BufferedWriter expects bytes, we need to convert our string to bytes
                    byte_data = ''.join(adjusted_boxes).encode()
                    bf.write(byte_data)
    ...
    def display_current_image(self):
        ...
        if image is None:
            ...
            return
        self.show_image(image)  # Updated to use your new show_image method



    def process_and_display_image(self, image, file_name):  # Define this method to process and display the image
        resized_image = self.resize_image(image, 800)
    ...
    def process_batch(self):
        ...
        self.label_progress.setRange(0, total_images)  # Set the range of the progress bar
        ...
        for idx, file_name in enumerate(self.image_files):
            ...
            if self.stop_batch or self.stop_labeling:  # Check for stop_labeling flag alongside stop_batch
                self.batch_running = False
                break
            file_path = os.path.join(self.image_directory, file_name)  # Changed to self.image_directory
            image = self.predict_and_draw_yolo_objects(cv2.imread(file_path), file_path)
            self.show_image(image)  # Updated to use your new show_image method
    ...
    def draw_point(self, image, x, y, color, radius):
    ...
    def predict(self, image, input_points, input_labels):
        self.predictor.set_image(image.copy())
        input_points_array = np.array(input_points)
        input_labels_array = np.array(input_labels)
        masks, scores, _ = self.predictor.predict(point_coords=input_points_array, point_labels=input_labels_array, multimask_output=True)
        ...
        for bb in self.current_bounding_boxes:
            cv2.rectangle(image, (bb[0], bb[1]), (bb[2], bb[3]), (0, 0, 200), 1)
        cv2.rectangle(image, (x, y), (x + w, y + h), (0, 255, 0), 1)
        self.temp_bbox = (x, y, x + w, y + h)
        cv2.putText(image, "AUTO LABELED", (20,20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
        for point in self.input_points:
            self.draw_point(image, point[0], point[1], (255, 0, 255), 2)
        return self.resize_image(image, 800), (x, y, w, h)
    ...
    def on_dino_label_clicked(self):
        # Ensure that the image directory has been selected
        if self.image_directory is not None:
            # If the directory is selected, pass it to the GroundingDINO function
            run_groundingdino(self.image_directory)
        else:
            # If the directory is not selected, prompt the user to select it
    ...
    def get_all_labels(self):
        ...
        for img_file in self.image_files:
            ...
            if os.path.exists(txt_file):
                # If the txt file exists, read it and add the labels to the all_labels list
                with open(txt_file, 'r') as f:
                    lines = f.readlines()
                    for line in lines:
                        # A each line starts with a class index
        ...
        for idx, image_file in enumerate(self.image_files):
            ...
            try:
                if self.model_type == 'yolov8':
                    # Process with YOLOv8's expected methods
                    results = self.model(image_file,half=True)
                elif self.model_type == 'yolov8_trt':
                    # Process with YOLOv8 TensorRT's expected methods
                    results = self.model.predict(source=image_file,int8=True)
                elif self.model_type == 'onnx':
                    # Process with ONNX's expected methods
                    # Make sure to not use half precision if the model expects float inputs
                    results = self.model.predict(source=image_file,int8=True)
                else:
                    raise ValueError(f"Error: Unknown model type '{self.model_type}'. Unable to process predictions.")
            except Exception as e:
                logging.error(f"Error during prediction: {e}")
                continue  # Skip this image and continue with the next

            # Handle pose, segmentation, or object detection based on the task
            try:
                if 'pose' in self.weights_file_path:
                    # Placeholder for keypoints
                    keypoints = results.keypoints
                    with open(label_file, 'w') as f:
                        for keypoint in keypoints:
                            # Assuming 'keypoint' contains coordinates and a confidence score
                            f.write(f"{keypoint.label} {keypoint.x} {keypoint.y} {keypoint.confidence}\n")

                elif 'seg' in self.weights_file_path:
                    # Extract segmentation masks
                    masks = results.masks
                    # Convert masks to NumPy array if not already (assuming masks are on CPU after results)
                    masks_np = masks.cpu().numpy() if not isinstance(masks, np.ndarray) else masks
                    # Overlay masks on the image
                    overlayed_image = overlay_masks_on_image(img, masks_np)
                    # Display the overlayed image in your GUI's image view
                    self.screen_view.setPixmap(QPixmap.fromImage(overlayed_image))
                else:
                    # Assuming the model returns bounding box predictions in this format
            ...
            except AttributeError:  # This will catch the '.pred' error for YOLOv8
...
# for blanks
    ...
    def slider_value_changed(self):
        ...
        # Update the threshold variables based on the current slider values:
        self.confidence_threshold = self.confidence_threshold_slider.value() / 100
        self.nms_threshold = self.nms_threshold_slider.value() / 100
    ...
    def display_stats(self):
        ...
        self.stats_widget = QWidget()  # Store widget as an attribute
        self.stats_widget.setWindowTitle("Labeling Statistics")

        # Create a QVBoxLayout to add your QTableView and other widgets (like buttons) if necessary
        layout = QVBoxLayout()

        # Create a QStandardItemModel and set the column headers
        model = QStandardItemModel()
        model.setHorizontalHeaderLabels(["Statistic", "Value"])

        # Add rows to your model with the statistics
        for key, value in stats.items():
            key_item = QStandardItem(str(key))
            value_item = QStandardItem(str(value))
            model.appendRow([key_item, value_item])

        # Create a QTableView, set the model, and add it to your layout
        table = QTableView()
        table.setModel(model)
        layout.addWidget(table)

        # Set the layout for your widget and show the widget
        self.stats_widget.setLayout(layout)
        self.stats_widget.show()

    def show_labeled_stats(self):
        # Create a list of image extensions
        image_extensions = ['.jpg', '.jpeg', '.png', '.bmp']

        # Only include files with the above extensions
        self.image_files = [f for f in self.image_files if any(
        ...
        # Add part to count class labels
        ...
        for image in self.image_files:
            label_file = self.get_label_file(image)
            if os.path.exists(label_file) and os.path.getsize(label_file) > 0:
                labeled_images += 1

                # Open the label file and read the number of lines (i.e., number of labels)
                with open(label_file, 'r') as f:
                    labels = f.readlines()
                    total_labels += len(labels)

        unlabeled_images = total_images - labeled_images
        labels_per_image = round(
            total_labels / total_images, 1) if total_images > 0 else 0
        labeling_progress = round(
            (labeled_images / total_images) * 100, 1) if total_images > 0 else 0
        ...
        stats = {
            ...
            # Include label counts per class
            **label_counts
        }

        # Add statistics to the settings
    ...
    def move_filtered_images(self, class_name):
        ...
        # Create a folder for the specified class
        class_folder = os.path.join(self.image_directory, class_name)
        os.makedirs(class_folder, exist_ok=True)

        # Move filtered images to the class folder
        for file_path in self.filtered_image_files:
        ...
        if self.filter_blanks_checkbox.isChecked():
            # Create a "blank" subfolder within the class folder
            blank_folder = os.path.join(class_folder, 'blank')
            os.makedirs(blank_folder, exist_ok=True)

            # Move blank images to the "blank" subfolder
            for file_path in self.filtered_image_files:
                ...
                # Check if the label file is blank (contains no annotations)
                if content == '':
                    file_name = os.path.basename(file_path)
                    dest_path = os.path.join(blank_folder, file_name)
                    try:
                        shutil.move(file_path, dest_path)
                        shutil.move(label_path, os.path.splitext(dest_path)[0] + '.txt')
                        print(f"Moved blank image {file_name} to {class_name}/blank folder.")
                    except Exception as e:
                        print(f"Error moving blank image {file_name}: {str(e)}")
    ...
    def open_image_video(self):
        ...
        if dir_name:
            ...
            self.saveSettings()  # save the settings aefter modifying it
            ...
            # Get all the image files in the directory
            self.image_files = sorted(glob.glob(os.path.join(dir_name, "*.[pP][nN][gG]")) +
            ...
            # Update the QLabel with the total number of imag

            if len(self.image_files) > 0:
                # Try to get the last opened image from the settings
                last_image = self.settings.get('lastImage', "")
                if last_image and last_image in self.image_files:
                    self.current_image_index = self.image_files.index(last_image)  # Corrected variable name
                else:
                    self.current_image_index = 0  # Reset the current image index

                self.current_file = self.image_files[self.current_image_index]  # Corrected variable name
    ...
    def create_bounding_box(self, x, y, w, h, label, confidence):
        ...
        if new_index != -1:
            ...
            bbox_drawer = BoundingBoxDrawer(
                x, y, w, h, self, class_id=new_index, confidence=confidence)

            # Check if the heads_area checkbox is checked
            if self.heads_area.isChecked() and new_index != 1:
                # Extract class ID from the QLineEdit widget
                try:
                    head_class_id = int(self.class_id.text())
                except ValueError:
                    print(
                        f"Warning: Invalid class ID '{self.class_id.text()}' provided in QLineEdit.")
                    head_class_id = 1  # Default to 1 or another value if the input is not a valid integer

                # Add an additional bounding box inside the existing bounding box with the specified class_ID
                head_x, head_y, head_w, head_h = self.calculate_head_area(
                    x, y, w, h)
                head_bbox_drawer = BoundingBoxDrawer(
                    head_x, head_y, head_w, head_h, self, class_id=head_class_id, confidence=confidence)

                # Add the head_bbox_drawer to the scene
    ...
    def load_bounding_boxes(self, label_file, img_width, img_height):
        # Load from txt file without confidence
        with open(label_file, "r") as f:
            bbox_strs = f.readlines()

        bbox_strs = [bbox_str for bbox_str in bbox_strs if bbox_str.strip()]
        bounding_boxes = [BoundingBox.from_str(
            bbox_str) for bbox_str in bbox_strs]
        bounding_boxes = [bbox for bbox in bounding_boxes if bbox is not None]

        # Load from json file with confidence
        label_file_json = label_file.replace('.txt', '.json')
        if os.path.exists(label_file_json):
            with open(label_file_json, "r") as f:
                data = json.load(f)

            # A order of bounding boxes is the same as in .txt file
            if isinstance(data, dict):
                ...
                print(
                    f"Unexpected data structure in {label_file_json}: {type(data)}")
                annotations = []

            for bbox, annotation in zip(bounding_boxes, annotations):
                # Add confidence to bounding boxes if it's available; otherwise, set it to 0
                bbox.confidence = annotation.get('confidence', 0)

        # Make sure to return a confidence for each bounding box, even if it wasn't set in the JSON file
        return [(bbox.to_rect(img_width, img_height), bbox.class_id, bbox.confidence if bbox.confidence is not None else 0) for bbox in bounding_boxes]
    ...
    def class_input_field_return_pressed(self):
        ...
        if new_class in existing_classes:
            # find index of new_class in the dropdown
...
# adjust and view labels for a single image
    ...
    def process_images_triggered(self):
        ...
        # Check if images_import is empty
        ...
        output_folder_name = "nonaugmented"  # default folder name

        # Create a list to store all selected effects
        selected_effects = []
        ...
        # Join selected effects with underscores or set to 'nonaugmented' if empty
    ...
    def apply_augmentations(self, current_image, image_path, label_path, output_path, output_folder_name, total_images):
        ...
        # Read YOLO label file and keep it the same.
    ...
    def ultralytics_train_clicked(self):
        ...
        # Determine if a pre-trained model file has been selected
        ...
        command = f"yolo {task} data={data_input} imgsz={imgsz_input} epochs={epochs_input} " \
    ...
    def handle_stdout(self):
        ...
        logger.info(data)  # Log to both file and console using your configured logger

        # Split the output into lines and append to a list
        output_lines = self.stdout_lines + data.split("\n")
        # Ensure only the last 10 lines are kept
        self.stdout_lines = output_lines[-10:]

        # Convert the lines back to a single string to display in the QLabel
        current_text = "<br>".join(self.stdout_lines)
        self.ultralyics_view.setText(current_text)

    def handle_stderr(self):
        data = self.process.readAllStandardError().data().decode()
        logger.error(data)  # Log to both file and console using your configured logger

        # Split the error output into lines and append to a list
        error_lines = self.stderr_lines + data.split("\n")
        # Ensure only the last 10 lines are kept
        self.stderr_lines = error_lines[-10:]

        # Convert the lines back to a single string to display in the QLabel
        current_text = "<br>".join(self.stderr_lines)
        self.ultralyics_view.setText(current_text)
    ...
    def load_model(self):
        fname, _ = QtWidgets.QFileDialog.getOpenFileName(self, 'Open file', 'c:\\', "Model files (*.pt)")
        if fname:
    ...
    def handle_conversion(self):
        ...
        format_selected = self.convert_model.currentText()  # convert_model is the name of your QComboBox
        try:
            self.model.export(format=format_selected,half=True,workspace=4,simplify=True)
            QMessageBox.information(self, "Conversion Successful", f'Model converted to {format_selected}')
        except Exception as e:
            QMessageBox.critical(self, "Conversion Error", str(e))
    ...
    def output_paths(self):
        ...
        if file:
            ...
            with open(data_file_path, "w") as f:
                ...
                f.write("train  = " + os.path.join(output_dir,
                        "train.txt").replace("/", "\\") + "\n")
                valid_path = "valid.txt" if use_valid_txt else "train.txt"
                f.write("valid  = " + os.path.join(output_dir,
                        valid_path).replace("/", "\\") + "\n")
                f.write("names = " + os.path.join(output_dir,
                        "obj.names").replace("/", "\\") + "\n")
                f.write("backup = " + os.path.join(output_dir,
                        "backup").replace("/", "\\") + "\n")
            ...
            with open(obj_yaml_file, "w", encoding="utf-8") as f:
                ...
                f.write("train: " + os.path.join(output_dir,
                        "train.txt").replace("\\", "/") + "\n")
                valid_path = "valid.txt" if use_valid_txt else "train.txt"
                f.write("val: " + os.path.join(output_dir,
                        valid_path).replace("\\", "/") + "\n\n")
                ...
                # save all your file paths in the settings dictionary
                ...
                # save it to the settings.json file
                self.saveSettings()

                QMessageBox.information(
                    self, 'Information', 'obj.data,obj.yaml,obj.names,backup and train.txt has been created!')
