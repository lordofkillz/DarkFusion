    ...
    def run(self):
        ...
        try:
            ...
            for i in range(0, total_images, self.batch_size):
                ...
                # This must return a list of (image_file, pil_image) for the batch
                ...
                for image_file, pil_image in processed_batch:
                    if pil_image is None or self.is_placeholder_file(image_file):
                        continue
                    thumbnail_path = os.path.join(
                        self.thumbnails_directory,
                        f"{os.path.splitext(os.path.basename(image_file))[0]}_thumb.jpeg"
                    )
                    try:
                        pil_image.save(thumbnail_path, "JPEG")
                    except Exception as e:
                        logger.error(f"Failed to save thumbnail for {image_file}: {e}")
                        continue
                    results.append((image_file, thumbnail_path))
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def process_images_cuda(self, image_files: List[str], batch_size: int, thumbnails_directory: str):
        import torch
        from PIL import Image
        from concurrent.futures import ThreadPoolExecutor
        import os
        ...
        for batch_index, batch in enumerate(batch_generator):
            ...
            with ThreadPoolExecutor(max_workers=8) as executor:
                processed_batch = list(executor.map(load_and_process, batch))

            # If you want to generate thumbnails for each box *here*, call create_thumbnail_for_label
            for image_file, pil_image in processed_batch:
                results.append((image_file, pil_image))  # The worker will do the thumbnail saving
    ...
    def process_images_lazy_batches(self, image_directory: str, batch_size: int, thumbnails_directory: str):
        # Gather image files
        image_files = [
        ...
        for batch_index, batch in enumerate(batch_generator):
            ...
            # Process thumbnail creation synchronously for each batch
            with ThreadPoolExecutor(max_workers=8) as executor:
                thumbnail_futures = []
                for image_file, pil_image in processed_batch:
                    if pil_image is None:
                        logger.warning(f"Skipping image due to load failure: {image_file}")
                        continue
                    if self.is_placeholder_file(image_file):
                        logger.info(f"Skipping placeholder: {image_file}")
                        continue
                ...
                # Wait until all thumbnails are generated before moving to the next batch
                concurrent.futures.wait(thumbnail_futures)

                # Explicitly rebuild preview list for processed images
    ...
    def delete_item(self, row, image_file, true_line_index, label_type):
        ...
        removed_line = lines.pop(true_line_index)
        self.save_label_lines(label_file, lines)

        thumbnail_filename = os.path.join(
            self.thumbnails_directory,
            f"{os.path.splitext(os.path.basename(image_file))[0]}_{true_line_index}.jpeg"
        )
        self.attempt_delete_thumbnail(thumbnail_filename)

        scene = self.screen_view.scene()
        for item in scene.items():
            if getattr(item, 'file_name', '') == image_file:
                scene.removeItem(item)

        QApplication.processEvents()

        # 🧼 Remove the row for this bounding box
        for r in range(self.preview_list.rowCount()):
            bbox_item = self.preview_list.item(r, 4)
            if bbox_item and bbox_item.data(Qt.UserRole + 2) == true_line_index:
                self.preview_list.removeRow(r)
                logger.info(f"Removed preview row {r} for bbox {true_line_index}")
                break

        # 🔁 Reload image and annotations fully to sync
    ...
    def navigate_frame(self, direction):
        # ---- Lock navigation if already busy ----
        ...
        try:
            ...
            if not self.is_placeholder_file(self.current_file):
                self.save_bounding_boxes(self.current_file, scene_width, scene_height)

            # ✅ Now switch the current file before loading new labels or preview
            self.current_file = new_file
            self.settings['lastImage'] = new_file
            self.saveSettings()

            # ✅ Show paginated label thumbnails for the new image!
            self.show_thumbnail_page(self.current_file, page=0)

            # If you have a display_image function that needs to update the big/central image:
            self.display_image(self.current_file, rebuild_preview=False)  # Don't trigger a preview rebuild here

            self.img_index_number.setValue(index + (1 if direction == 'next' else -1))

            # Safe call to load_labels only if label_file is valid
            if self.label_file and not self.is_placeholder_file(self.label_file):
...
def run_pyqt_app():
    ...
    window = MainWindow()
    window.show()  # Only show one instance!
