...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def delete_item(self, row, image_file, true_line_index, label_type):
        """Delete a label and fully re-align all thumbnails and preview rows for this image."""

        if self.is_placeholder_file(image_file):
            logger.info(f"Skipping deletion for placeholder file: {image_file}")
            return

        # 1. Load and update label lines
        label_file = os.path.splitext(image_file)[0] + '.txt'
        ...
        # Remove the label line and save updated lines
        lines.pop(true_line_index)
        self.save_label_lines(label_file, lines)

        # 2. Remove ALL thumbnails for this image (to avoid any out-of-sync thumbnails)
        base_name = os.path.splitext(os.path.basename(image_file))[0]
        if hasattr(self, 'thumbnails_directory') and os.path.exists(self.thumbnails_directory):
            for fname in os.listdir(self.thumbnails_directory):
                if fname.startswith(base_name + "_") and fname.endswith(".jpeg"):
                    try:
                        os.remove(os.path.join(self.thumbnails_directory, fname))
                    except Exception as e:
                        logger.warning(f"Could not delete thumbnail {fname}: {e}")

        # 3. Regenerate thumbnails for the new, updated list of label lines
        ...
        # 4. Rebuild the preview list for this image only (fresh thumbnails, correct lines)
        self._run_debounced_thumbnail_page_now(image_file, page=0)

        # 5. Redisplay image with updated boxes
        self.display_image(image_file)

        # 6. Sync list selection
        self.sync_list_view_selection(image_file)

        logger.info(f"Deleted bounding box {true_line_index} from {image_file}. Thumbnails and preview list now in sync.")
