...
class SettingsDialog(QtWidgets.QDialog):
    ...
    def init_class_tab(self):
        ...
        for btn in [self.add_btn, self.rename_btn, self.remove_btn, self.search_btn]:
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def eventFilter(self, source, event):
        try:
            # 🟢 Handle mouse clicks as you already do
                ...
                if item is not None:
                    ...
                    if hasattr(self, "_last_click_time") and hasattr(self, "_last_button_clicked"):
                        ...
                        if elapsed_time < 0.3 and self._last_button_clicked == current_button:
                            ...
                            return True  # Consume event, too soon same button click
            ...
            # 🟢 NEW: Handle key presses inside preview list for navigation
    ...
    def handle_right_click(self, index):
        ...
        row = index.row()
        if row >= self.preview_list.rowCount():
            logger.warning(f"Right-click ignored — invalid row index: {row}")
            return
        ...
        try:
            ...
            true_line_index = label_item.data(Qt.UserRole + 2)
            self.delete_item(row, image_file, true_line_index, label_type)

        except Exception as e:
            logger.error(f"[handle_right_click] Unexpected error: {str(e)}")
    ...
    def delete_item(self, row, image_file, true_line_index, label_type):
        """Delete a label and precisely realign thumbnails and preview rows for just this image."""
        ...
        # Remove ONLY the deleted thumbnail
        thumbnail_to_delete = os.path.join(self.thumbnails_directory, f"{base_name}_{true_line_index}.jpeg")
        if os.path.exists(thumbnail_to_delete):
            try:
                os.remove(thumbnail_to_delete)
                logger.info(f"Deleted thumbnail: {thumbnail_to_delete}")
            except Exception as e:
                logger.warning(f"Could not delete thumbnail {thumbnail_to_delete}: {e}")

        # Regenerate thumbnails and refresh preview_list rows for THIS IMAGE ONLY
        pixmap = QPixmap(image_file)
        img_width, img_height = pixmap.width(), pixmap.height()

        # Clear ONLY rows for this image
        rows_to_remove = []
        for i in range(self.preview_list.rowCount()):
            if self.preview_list.item(i, 0).text() == image_file:
                rows_to_remove.append(i)
        for i in reversed(rows_to_remove):
            self.preview_list.removeRow(i)

        # Now regenerate fresh thumbnails and rows
        ...
        logger.info(f"Deleted bounding box {true_line_index} from {image_file}. Thumbnails and preview list realigned.")
    ...
    def show_thumbnail_page(self, image_file, page=0):
        """Debounced thumbnail loader: only shows for last request after fast skips."""
        self._pending_thumbnail_args = (image_file, page)
        self._thumbnail_timer.start(80)  # 80ms debounce—tweak for UX
    ...
    def auto_label_images2(self):
        ...
        # Get batch size from your spinbox
        batch_size = self.batch_inference.value()

        # Keypoint logic unchanged
        num_expected_keypoints = self.keypoint_list.rowCount()
        ...
        for batch_start in range(0, total_images, batch_size):
            ...
            # Remove Nones and their files/whs in case of preprocessing failure
            ...
            try:
                ...
                for i, image_file in enumerate(valid_batch_files):
    ...
    def display_image(self, file_name=None, image=None, rebuild_preview=True, page=0):
        ...
        """
        # ---- 1. Display (and paginate) thumbnail preview table ----
        if rebuild_preview and file_name and not self.is_placeholder_file(file_name):
        ...
        if image is not None:
            ...
            # 👉 NO call to show_thumbnail_page here anymore!
