...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def display_all_images(self):
        ...
        # Check explicitly if placeholder exists
        if not os.path.exists(placeholder_path):
            logger.error(f"Placeholder image not found at: {placeholder_path}")
            return

        image_files = [
            f for f in self.get_image_files(self.image_directory)
            if not self.is_placeholder_file(f)
        ]

        # Sort images by their label file size (largest first)
        ...
        # Insert placeholder explicitly
        self.filtered_image_files = [placeholder_path] + sorted_images

        # The ONLY place to set ListView + total_images:
        self.update_list_view(self.filtered_image_files)   # <<< THIS updates the label

        # Clear annotations, show placeholder, etc
