...
from PyQt5.QtWidgets import (QApplication, QFileDialog,
                             ...
                             QTableWidgetItem, QColorDialog, QMenu,QSplashScreen,QTableView, QVBoxLayout,QWidget,QHeaderView,QStyledItemDelegate,QStyle,QTabWidget,QStyleOptionButton,QGraphicsPolygonItem,QGraphicsEllipseItem,QCheckBox,QAbstractItemView,QProgressDialog)
...
from dinov5_2 import run_groundingdino
...
def get_color(class_id, num_classes, alpha=150, class_names=None):
    ...
    # If class_names list is available, try to use class name as key
    class_key = f"classColor_{class_id}"
    if class_names and 0 <= class_id < len(class_names):
        class_key = f"classColor_{class_names[class_id]}"

    # Try loading saved color from settings
    try:
        ...
        print(f"[get_color] Couldn't load color for {class_key}: {e}")
...
def reflect_pad_pil(img, pad_right, pad_bottom):
    # Pads image using reflect mode (mirror padding)
    np_img = np.array(img)
    if pad_right > 0:
...
def load_and_process(image_file):
    ...
    try:
        ...
        return (image_file, img_pil)
    except Exception as e:
        print(f"[process_images_cuda] Error processing {image_file}: {e}")
...
class CustomGraphicsView(QGraphicsView):
    ...
    def _save_and_play_sound(self):
        ...
        # 🔁 Immediately sync thumbnails and preview
        current_page = getattr(self.main_window, "current_page", 0)
        self.main_window.show_thumbnail_page(self.main_window.current_file, page=current_page)
        self.main_window.display_image(self.main_window.current_file, rebuild_preview=False)
...
class BoundingBoxDrawer(QGraphicsRectItem):
    ...
    def __init__(self, x, y, width, height, main_window, class_id=None, confidence=None, unique_id=None):
        ...
        # Cache color and pen
        class_names = [main_window.classes_dropdown.itemText(i) for i in range(main_window.classes_dropdown.count())]
...
class KeypointDrawer(QGraphicsItem):
    ...
    def paint(self, painter, option, widget):
        ...
        # Define your skeleton connection map (customize this as needed)
        SKELETON_CONNECTIONS = [
            (5, 7), (7, 9),     # left arm
            (6, 8), (8, 10),    # right arm
            (11, 13), (13, 15), # left leg
            ...
            (5, 6),             # shoulders
            (11, 12),           # hips
            (5, 11), (6, 12),   # torso connection
        ]

        # First draw lines (skeleton)
        for i1, i2 in SKELETON_CONNECTIONS:
            ...
            skeleton_pen = QPen(QColor(255, 255, 255, 180), 1.5 / zoom_factor)
...
class BoundingBox:
    ...
    def to_xyxy(self, img_width, img_height):
        # Returns absolute (x1, y1, x2, y2) from normalized center/width/height
...
class SettingsDialog(QtWidgets.QDialog):
    ...
    def init_class_tab(self):
        ...
        self.class_list.setSelectionMode(QtWidgets.QAbstractItemView.ExtendedSelection)  # Enable multi-selection
        self.class_list.itemClicked.connect(self.sync_dropdown_to_class_list)
        self.class_list.currentItemChanged.connect(self.sync_dropdown_to_class_list)
        ...
        self.class_input.setPlaceholderText("Enter class name or search")
        input_layout.addWidget(self.class_input, 3)

        # Button group layout
        button_group = QtWidgets.QHBoxLayout()
        self.add_btn = QtWidgets.QPushButton("Add")
        self.rename_btn = QtWidgets.QPushButton("Rename")
        self.remove_btn = QtWidgets.QPushButton("Remove")
        self.search_btn = QtWidgets.QPushButton("Search")
        self.search_btn.setToolTip("Jump to class in list")
        self.merge_btn = QtWidgets.QPushButton("Merge")
        self.merge_btn.setToolTip("Select multiple classes (Ctrl+Click) then merge into one")

        for btn in [self.add_btn, self.rename_btn, self.remove_btn, self.search_btn, self.merge_btn]:
            btn.setMinimumWidth(70)
            button_group.addWidget(btn)

        # Combine into main controls row
        ...
        self.search_btn.clicked.connect(self.search_class_in_list)
        self.merge_btn.clicked.connect(self.merge_classes)
    ...
    def search_class_in_list(self):
        search_term = self.class_input.text().strip().lower()
        if not search_term:
            return
        ...
        for i in range(self.class_list.count()):
            ...
            if search_term in item.text().lower():
                ...
                self.class_list.scrollToItem(item, QtWidgets.QAbstractItemView.PositionAtCenter)
                item.setSelected(True)
                self.sync_dropdown_to_class_list(item)  # 🔥 Sync dropdown too
                return

        QtWidgets.QMessageBox.information(self, "Not Found", f"No class found matching: {search_term}")

    def sync_dropdown_to_class_list(self, item):
        """
        When a class is selected in the list, update the main class dropdown.
        """
        # Handle None item (when list selection is cleared)
        if item is None:
            return
        
        class_text = item.text().split(": ", 1)[-1]  # e.g., "2: car" → "car"
        dropdown = self.parent().classes_dropdown
        index = dropdown.findText(class_text)
        if index != -1:
            dropdown.setCurrentIndex(index)
            logger.info(f"✅ Synced dropdown to class '{class_text}' at index {index}")
    ...
    def rebuild_hotkeys_ui(self):
        ...
        for i, class_name in enumerate(classes):
            ...
            # Use QPushButton for color (so it's clickable)
            color_btn = QPushButton()
            color_btn.setFixedSize(QSize(30, 20))
            saved_color = self.parent().settings.get(f'classColor_{class_name}', color.name())
            color_btn.setStyleSheet(f"background-color: {saved_color}; border: 1px solid #333;")
            color_btn.clicked.connect(lambda _, btn=color_btn, cls=class_name: self.pick_color(btn, cls))
            self.hotkey_grid.addWidget(color_btn, row, 2)
            self.classColorPickers[class_name] = color_btn

            # Reset button
            reset_btn = QPushButton("Reset")
            reset_btn.setFixedWidth(50)
            reset_btn.clicked.connect(lambda _, cls=class_name, btn=color_btn: self.reset_color(cls, btn))
            self.hotkey_grid.addWidget(reset_btn, row, 3)
    ...
    def remove_class(self):
        ...
        # Get the current row before removal
        ...
        # Select a nearby item after removal
        if self.class_list.count() > 0:
            # If we removed the last item, select the new last item
            if current_row >= self.class_list.count():
                new_row = self.class_list.count() - 1
            else:
                # Otherwise, select the item at the same position
                new_row = current_row
            
            self.class_list.setCurrentRow(new_row)
            # Manually trigger sync since the item might be the same position but different class
            if self.class_list.currentItem():
                self.sync_dropdown_to_class_list(self.class_list.currentItem())
    ...
    def merge_classes(self):
        """Merge multiple selected classes into one, updating all label files"""
        selected_items = self.class_list.selectedItems()
        
        if len(selected_items) < 2:
            QtWidgets.QMessageBox.warning(self, "Selection Error", 
                                        "Please select at least 2 classes to merge (Ctrl+Click for multiple selection)")
            return
        ...
        for item in selected_items:
            ...
            class_name = text.split(": ", 1)[-1]
            selected_data.append((idx, class_name))
        
        selected_data.sort(key=lambda x: x[0])  # Sort by index
        
        # Create merge dialog
        merge_dialog = MergeClassDialog(selected_data, self)
        if merge_dialog.exec_() == QtWidgets.QDialog.Accepted:
            target_idx, target_name = merge_dialog.get_target_class()
            
            # Get indices to be merged
            indices_to_merge = [idx for idx, name in selected_data if idx != target_idx]
            
            # Update label files
            success = self.update_label_files_for_merge(target_idx, indices_to_merge)
            
            if success:
                # Remove merged classes from classes.txt
                classes = self.parent().load_classes()
                new_classes = []
                for i, cls in enumerate(classes):
                    if i == target_idx or i not in indices_to_merge:
                        new_classes.append(cls)
                
                # Update class at target index if name changed
                if target_idx < len(new_classes):
                    new_classes[target_idx] = target_name
                
                self._save_classes(new_classes)
                self.parent().update_classes_dropdown(new_classes)
                self.refresh_class_list()
                self.rebuild_hotkeys_ui()
                
                QtWidgets.QMessageBox.information(self, "Success", 
                    f"Successfully merged {len(indices_to_merge)} classes into '{target_name}'")

    def update_label_files_for_merge(self, target_idx, indices_to_merge):
        """Update all label files to merge class indices"""
        from pathlib import Path
        
        directory = self.parent().image_directory or self.parent().output_path or os.getcwd()
        
        # Find all label files
        label_files = list(Path(directory).glob("*.txt"))
        
        # Remove classes.txt from the list
        ...
        try:
            for label_file in label_files:
                ...
                for line in lines:
                    ...
                    parts = line.split()
                    if len(parts) >= 5:  # class_id x y w h [confidence]
                        class_id = int(parts[0])
                        
                        # If this class is being merged, change to target
                        if class_id in indices_to_merge:
            ...
            logger.info(f"Updated {updated_count} label files for class merge")
            return True
            
        except Exception as e:
            logger.error(f"Error updating label files: {e}")
            QtWidgets.QMessageBox.critical(self, "Error", f"Failed to update label files: {e}")
            return False
    ...
    def check_annotation_file(self, file_path, image_folder):
        ...
        with open(file_path, 'r') as f:
            for line in f:
                ...
                # ✅ Track usage
                self.used_class_indices.add(bbox.class_id)
    ...
    def scan_annotations(self):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for file_name in txt_files_set:
                futures.append(executor.submit(self.process_files, file_name, annotation_folder, image_folder, review_folder, statistics))
            for future in futures:
                # collect used class IDs from each future
                try:
                    result = future.result()
                    if isinstance(result, set):
                        used_class_indices.update(result)
                except Exception as e:
                    logging.error(f"Thread error during processing: {e}")
        ...
        all_class_ids = set(self.valid_classes) if isinstance(self.valid_classes, list) else set(self.valid_classes.keys())
        unused_classes = sorted(set(self.valid_classes) - self.used_class_indices)

        if unused_classes:
            logging.warning(f"The following class IDs are defined but unused: {unused_classes}")
            statistics["unused_classes"] = [f"Class ID {cls_id}" for cls_id in unused_classes]
...
class TileWorker(QThread):
    ...
    def __init__(self, directory, tile_width, tile_height, overlap_ratio=0.5):
        super().__init__()
        self.directory = directory
        self.tile_width = tile_width
        self.tile_height = tile_height
        self.overlap_ratio = overlap_ratio  # e.g. 0.5 = 50% overlap

    def run(self):
        image_files = [f for f in os.listdir(self.directory) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
        total = len(image_files)
        tiled_dir = os.path.join(self.directory, 'tiles')
        ...
        for i, filename in enumerate(image_files):
            ...
            stride_x = int(self.tile_width * (1 - self.overlap_ratio))
            stride_y = int(self.tile_height * (1 - self.overlap_ratio))
            stride_x = max(1, stride_x)
            stride_y = max(1, stride_y)

            # Generate tiles, hugging right and bottom edges
            tile_id = 0
            for y1 in range(0, img_h, stride_y):
                if y1 + self.tile_height > img_h:
                    y1 = img_h - self.tile_height
                y1 = max(0, y1)
                y2 = min(y1 + self.tile_height, img_h)
                for x1 in range(0, img_w, stride_x):
                    if x1 + self.tile_width > img_w:
                        x1 = img_w - self.tile_width
                    x1 = max(0, x1)
                    ...
                    # Only pad if needed (should be minimal with overlap)
                    ...
                    out_w, out_h = self.tile_width, self.tile_height

                    # Adjust labels for this tile
                    tile_labels = []
                    for bbox in bboxes:
                        adj = self.adjust_bbox_for_tile(
                            bbox, x1, y1, tile_w, tile_h, out_w, out_h, img_w, img_h
                        )
                        if adj is not None and adj.width > 0.01 and adj.height > 0.01:
                            tile_labels.append(adj.to_str())

                    tile_base = filename.rsplit('.', 1)[0] + f"_tile_{tile_id}"
                    tile_img.save(os.path.join(tiled_dir, f"{tile_base}.png"))
                    with open(os.path.join(tiled_dir, f"{tile_base}.txt"), 'w') as labelf:
                        labelf.write('\n'.join(tile_labels))
                    tile_id += 1

                # Ensure last row is not missed (bottom hugging)
                if y1 + self.tile_height >= img_h:
                    break
    ...
    def adjust_bbox_for_tile(
            self, bbox, tile_x1, tile_y1, tile_w, tile_h, out_w, out_h, orig_w, orig_h,
            min_area_ratio=0.25  # 25% of the original bbox area must remain
        ):
        # Convert to absolute box in original image
        abs_x1 = (bbox.x_center - bbox.width / 2) * orig_w
        abs_y1 = (bbox.y_center - bbox.height / 2) * orig_h
        abs_x2 = (bbox.x_center + bbox.width / 2) * orig_w
        abs_y2 = (bbox.y_center + bbox.height / 2) * orig_h
        ...
        # --- NEW: Check intersection area vs. original area ---
        orig_area = max(0, abs_x2 - abs_x1) * max(0, abs_y2 - abs_y1)
        ...
        # Normalize to [0,1] in output tile
        norm_x1 = tile_box_x1 / out_w
        norm_y1 = tile_box_y1 / out_h
        norm_x2 = tile_box_x2 / out_w
        norm_y2 = tile_box_y2 / out_h

        new_x_center = (norm_x1 + norm_x2) / 2
        new_y_center = (norm_y1 + norm_y2) / 2
...
class ExtractionWorker(QObject):
    progress = pyqtSignal(int)
    finished = pyqtSignal(list)  # Emits list of (image_file, thumbnail_path)
    error = pyqtSignal(str)

    def __init__(self, image_files, thumbnails_directory, batch_size, process_images_cuda):
        super().__init__()
        self.image_files = image_files
        self.thumbnails_directory = thumbnails_directory
        self.batch_size = batch_size
        self.process_images_cuda = process_images_cuda

    def is_placeholder_file(self, file_path):
        basename = os.path.basename(file_path).lower()
        return basename in {"default.png", "default_temp.png"}

    def run(self):
        try:
            total_images = len(self.image_files)
            processed_count = 0
            # Still loop over batches for progress, but don't save any full-image thumbnail!
            for i in range(0, total_images, self.batch_size):
                batch_images = self.image_files[i:i + self.batch_size]
                processed_batch = self.process_images_cuda(
                    batch_images,
                    self.thumbnails_directory
                )
                # We don't do anything here; thumbnails are created by process_images_cuda pipeline (cropped only)
                processed_count += len(batch_images)
                self.progress.emit(processed_count)
            # Results: (image_file, None) just to keep signal happy (can be changed if you need actual cropped paths)
            self.finished.emit([(img, None) for img in self.image_files])
        except Exception as e:
            self.error.emit(str(e))
...
class ThumbnailBatchWorker(QObject):
    finished = pyqtSignal()
    progress = pyqtSignal(int)

    def __init__(self, image_directory, batch_size, thumbnails_directory, process_images_lazy_batches):
        super().__init__()
        self.image_directory = image_directory
        self.batch_size = batch_size
        self.thumbnails_directory = thumbnails_directory
        self.process_images_lazy_batches = process_images_lazy_batches

    def run(self):
        # Call the passed process_images_lazy_batches function
        self.process_images_lazy_batches(
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        ui_path = Path(__file__).resolve().parent / "UltraDarkFusion_v5.2.ui"
        ...
        self.speed_levels = [0.25, 0.5, 1.0, 1.5, 2.0, 4.0] 
        self.page_size = 100
        self.page_size = 100  # Or whatever feels fast/responsive
        self.current_page = 0
        ...
        for child in self.findChildren(QtWidgets.QWidget):
            ...
            # ✅ Optional: extract whatsThis if used
    ...
    def change_language(self, index):
        ...
        if texts_to_translate:
            ...
            worker.signals.error.connect(lambda text, e: print(f"⚠️ Error translating '{text}': {e}"))
            self.threadpool.start(worker)
        else:
            print("✅ Loaded entirely from cache")
    ...
    def update_ui_from_cache(self):
        for item in self.widgets_to_translate:
            widget_type, widget = item[0], item[1]
            key = item[-1]
            translated_text = self.translation_cache.get(key)
            if translated_text:
                self.update_widget_text_by_key(key, translated_text)
    ...
    def crop_images(self):
        ...
        # Call the tile worker, not crop worker!
        self.worker = TileWorker(self.directory, width, height)
        self.worker.update_progress.connect(self.label_progress.setValue)
        self.worker.start()
    ...
    def draw_roi(self, width, height, x, y, roi_id):
        ...
        # Use sip.isdeleted() to safely check if the item was deleted
    ...
    def polygon_to_mask_shapely(self, segmentation, img_width, img_height):
        """
        Converts a normalized YOLO-style polygon to a binary mask using Shapely for validation + OpenCV for rasterizing.
        """
        if len(segmentation) < 6 or len(segmentation) % 2 != 0:
            logging.warning("Invalid segmentation length. Must be even and >= 6.")
        ...
        try:
            ...
            if not polygon.is_valid:
                polygon = polygon.buffer(0)  # attempt to fix self-intersection

            if not polygon.is_valid or polygon.is_empty:
                logging.warning("Polygon invalid or empty after Shapely fix.")
                return np.zeros((img_height, img_width), dtype=bool)

            # Get exterior coordinates for drawing
            ...
            logging.error(f"Failed to convert polygon to mask with Shapely: {e}")
            return np.zeros((img_height, img_width), dtype=bool)
        ...
        for yolo_line in yolo_lines:
            try:
                ...
                if box_obj.segmentation:
                    mask = None
                    try:
                        mask = self.polygon_to_mask_shapely(box_obj.segmentation, img_width, img_height)
                    except Exception as e:
                        logging.warning(f"Shapely mask generation failed: {e}")
                    if mask is None or not mask.any():
                        try:
                            mask = self.polygon_to_mask(box_obj.segmentation, img_width, img_height)
                        except Exception as e2:
                            logging.error(f"OpenCV fallback failed too: {e2}")
                            continue

                    polygon = self.get_polygon_from_mask(mask)
                    if not polygon:
                        logging.warning(f"Failed to get polygon from mask for {box_obj}: {mask}")
                        continue
                    ...
                    if masks is None or len(masks) == 0:
                        logging.warning("No masks returned by SAM.")
                        continue

                    # Morphological cleanup
                    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
                    refined_masks = [
                        cv2.morphologyEx((m > 0).astype(np.uint8) * 255, cv2.MORPH_CLOSE, kernel)
                        for m in masks
                    ]
                    refined_masks = [
                        cv2.morphologyEx(m, cv2.MORPH_OPEN, kernel) for m in refined_masks
                    ]

                    # Choose best SAM mask by area
                    sam_mask = max(refined_masks, key=lambda m: np.sum(m > 0))

                    # 🧠 Combine with GrabCut for occlusion robustness
                    grabcut_mask = self.refine_with_grabcut(preprocessed_image, [x_min, y_min, x_max, y_max])
                    fused_mask = cv2.bitwise_or(sam_mask, grabcut_mask)
                    sam2_mask = self.get_sam2_mask(preprocessed_image, (x_min, y_min, x_max, y_max), model_path="Sam/sam2.1_l.pt")

                    area_fused = np.sum(fused_mask > 0)
                    area_sam2 = np.sum(sam2_mask > 0) if sam2_mask is not None else 0

                    if area_sam2 > area_fused:
                        fused_mask = (sam2_mask.astype(np.uint8) * 255)


                    # 🧪 Optional: edge filter — cut weak outer borders
                    edges = cv2.Canny(preprocessed_image, 50, 150)
                    fused_mask[edges == 0] = 0

                    # Check if mask is still good; if not, fallback to shadow-based extractor
                    polygon = self.get_polygon_from_mask(fused_mask, simplify=True, epsilon_factor=0.01)

                    if polygon and len(polygon) >= 4:
                        # Good mask → use fused mask directly
                        mask = fused_mask.astype(bool)
                    else:
                        logging.warning("🪫 Weak SAM+GrabCut result, using shadow fallback.")
                        region = preprocessed_image[y_min:y_max, x_min:x_max]
                        fallback_mask = self.extract_shadow_mask(region)

                        # Build a full-sized mask with the fallback
                        mask = np.zeros_like(fused_mask, dtype=bool)
                        mask[y_min:y_max, x_min:x_max] = fallback_mask
    ...
    def apply_copy_paste_augmentation(self, progress_offset=0):
        """
        Performs smart copy-paste augmentation.
        Tries to paste objects of the same class into images where they already exist.
        ...
        # Build class-to-box cache
        class_to_boxes = {}
        for src_img_path in self.image_files:
            ...
            for ann in src_anns:
                box = BoundingBox.from_str(ann)
                if not box or box.segmentation or box.obb:
                    continue
                class_to_boxes.setdefault(box.class_id, []).append((src_img_path, box))
        ...
        for idx, image_file in enumerate(self.image_files):
            ...
            for _ in range(objects_to_add):
                ...
                src_img_path, bbox = candidate_boxes[np.random.randint(len(candidate_boxes))]
    ...
    def generate_segmentation_mask(self, image, bbox):
        ...
        Compares SAM1 + morphology vs. Ultralytics SAM2 and selects the better mask.
        """
        ...
        try:
            ...
            if masks is not None and len(masks) > 0:
                refined_masks = []
                for mask in masks:
                    rm = cv2.morphologyEx((mask > 0).astype(np.uint8) * 255, cv2.MORPH_CLOSE, kernel)
                    rm = cv2.morphologyEx(rm, cv2.MORPH_OPEN, kernel)
                    refined_masks.append(rm)
                sam1_mask = max(refined_masks, key=lambda m: np.sum(m > 0))
        except Exception as e:
            logging.error(f"SAM1 mask generation failed: {e}")

        # --- SAM2 logic ---
        sam2_mask = None
        try:
            sam2_mask = self.get_sam2_mask(image, bbox=(x_min, y_min, x_max, y_max), model_path="Sam/sam2.1_l.pt", device="cuda")
            if sam2_mask is not None:
                sam2_mask = (sam2_mask.astype(np.uint8) * 255)
        except Exception as e:
            logging.error(f"SAM2 mask generation failed: {e}")

        # --- Compare both ---
        area1 = np.sum(sam1_mask > 0) if sam1_mask is not None else 0
        area2 = np.sum(sam2_mask > 0) if sam2_mask is not None else 0

        best_mask = sam1_mask if area1 >= area2 else sam2_mask

        if best_mask is not None:
            return best_mask.astype(bool)
        else:
            logging.warning("No valid mask returned by either SAM1 or SAM2.")
            return None
    ...
    def draw_mask_and_bbox(self, image, mask, class_id=None, num_classes=1, original_bbox=None, obb_points=None, prioritize="largest"):
            """
            Draws the input binary mask filled with class color, and overlays bounding boxes.
            """
            img_height, img_width = image.shape[:2]
            ...
            # 📸 Overlay the full mask directly
            overlay = image.copy()
            alpha = 0.4
            ...
            # 🔲 Bounding box from mask (no more contour selection here)
            ...
            if self.is_obb_generation_enabled:
                if obb_points and len(obb_points) == 8:
                    ...
                    # Fallback: get min area rect from mask itself
    ...
    def process_batch(self):
        ...
        for idx, image_file in enumerate(self.image_files):
            ...
            # 🔁 Generate masks for noise/shadow
            masks_for_bboxes = []
            for line in labels:
                try:
                    box_obj = BoundingBox.from_str(line)
                    if not box_obj or box_obj.segmentation or box_obj.obb:
                        continue
                    voc = pbx.convert_bbox(
                        (box_obj.x_center, box_obj.y_center, box_obj.width, box_obj.height),
                        from_type="yolo", to_type="voc",
                    ...
                    x_min, y_min, x_max, y_max = map(int, map(round, voc))

                    with torch.amp.autocast('cuda'):
                        masks, _, _ = self.predictor.predict(
                            box=np.array([[x_min, y_min, x_max, y_max]]), multimask_output=False)
                    if masks is None or len(masks) == 0:
                        continue
                    mask = (masks[0] > 0).astype(bool)
                    masks_for_bboxes.append((mask, (x_min, y_min, x_max, y_max)))
                except Exception as e:
                    logging.error(f"Failed to get mask for noise/shadow: {e}")

            # 🛠 Actually apply them now
            if 'noise' in dirs:
    ...
    def convert_bbox_to_segmentation(self, image, annotation_file):
        ...
        Applies SAM + GrabCut + edge filtering and fallback refinement.
        """
        img_height, img_width = image.shape[:2]
        self.predictor.set_image(image)
        segmentation_labels = []
        ...
        try:
            ...
            logging.error(f"Failed to read label file: {annotation_file} - {e}")
        ...
        for yolo_line in yolo_lines:
            try:
                ...
                if box_obj.segmentation or box_obj.obb:
                    continue
                ...
                refined_masks = [cv2.morphologyEx(m, cv2.MORPH_OPEN, kernel) for m in refined_masks]
                # Generate both SAM1 and GrabCut fused mask
                sam_mask = max(refined_masks, key=lambda m: np.sum(m > 0))
                grabcut_mask = self.refine_with_grabcut(image, [x_min, y_min, x_max, y_max])
                fused_sam1_mask = cv2.bitwise_or(sam_mask, grabcut_mask)

                # Generate SAM2 mask (Ultralytics-native)
                sam2_mask = self.get_sam2_mask(image, (x_min, y_min, x_max, y_max), model_path="Sam/sam2.1_l.pt", device="cuda")
                fused_sam2_mask = (sam2_mask.astype(np.uint8) * 255) if sam2_mask is not None else None

                # Compare by area (or other quality score later)
                area1 = np.sum(fused_sam1_mask > 0)
                area2 = np.sum(fused_sam2_mask > 0) if fused_sam2_mask is not None else 0

                # Select best
                fused_mask = fused_sam1_mask if area1 >= area2 else fused_sam2_mask


                # Edge filter for outer contour clarity
                edges = cv2.Canny(image, 50, 150)
                fused_mask[edges == 0] = 0

                polygon = self.get_polygon_from_mask(fused_mask, simplify=False, epsilon_factor=0.02)

                # Fallback to shadow if polygon is too weak
                if not polygon or len(polygon) < 4:
                    logging.info("Fallback to shadow extractor.")
                    region = image[y_min:y_max, x_min:x_max]
                    fallback_mask = self.extract_shadow_mask(region)
                    fused_mask = np.zeros_like(sam_mask)
                    fused_mask[y_min:y_max, x_min:x_max] = fallback_mask
                # Use same refined mask logic as draw_mask_and_bbox
                kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
                ...
                # Then extract polygon from the now-clean mask
                polygon = self.get_polygon_from_mask(fused_mask, simplify=True, epsilon_factor=0.001)

                if not polygon or len(polygon) < 3:
                    continue

                # Normalize polygon coordinates
                simplified = np.array(polygon, dtype=np.float32)
    ...
    def extract_shadow_mask(self, region):
        """Rough mask based on grayscale threshold, like your shadow logic."""
    ...
    def get_sam2_mask(self, image, bbox, model_path="Sam/sam2.1_l.pt", device='cuda'):
        """
        Uses Ultralytics SAM 2.1 model to predict a binary mask for a given bounding box.
        """
        # Lazy-load SAM2
        if not hasattr(self, "_sam2_model"):
            if not os.path.exists(model_path):
                raise FileNotFoundError(f"SAM2 model not found at {model_path}")
            self._sam2_model = SAM(model_path)
        ...
        try:
            with torch.amp.autocast(device_type=device):
                # ✅ 'conf' is now used to feed bboxes
                results = model.predict(image, conf=0.25, bboxes=yolo_box, show_boxes=False, verbose=False)

            if results and results[0].masks is not None:
                return results[0].masks.data[0].cpu().numpy().astype(bool)

        except Exception as e:
            logging.error(f"SAM2 inference failed: {e}")
    ...
    def stop_program(self):
        ...
        # Force-select "null" without triggering on_input_source_changed loop
        ...
        logger.warning("🛑 Processing stopped and input source reset to 'null'.")
    ...
    def on_input_source_changed(self, index):
        if not hasattr(self, 'ui_initialized') or not self.ui_initialized:
            ...
            return
        # Only stop ongoing video/camera capture without switching input to "null"
        if hasattr(self, 'timer2') and self.timer2 is not None:
            self.timer2.stop()
            self.timer2.timeout.disconnect()
            self.timer2 = None
    ...
    def capture_with_mss(self):
        if not hasattr(self, '_mss_instance') or not hasattr(self, '_mss_monitors'):
            self.setup_mss()
    ...
    def on_extract_button_clicked(self):
        ...
        self.playback_extraction_enabled = False  # 🛑 Disable playback saving during manual extract
        ...
        try:
            # === Try extracting from video file ===
            ...
            if video_path and os.path.exists(video_path):
                ...
                return  # Done, cleanup will happen in finally

            # === Fallback to webcam or desktop ===
            ...
            # 🔥 Prompt for custom session name
            session_name, ok = QInputDialog.getText(self, "Session Name", f"Enter a name for this extract session (e.g. 'bo6_nuke'):")
            if not ok or not session_name.strip():
                session_name = current_source.replace(" ", "_")

            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            self.video_name = f"{session_name.strip()}_{timestamp}"
            self.output_subfolder = os.path.join(self.output_path, f"{self.video_name}_Frames")
            ...
            # Only show message if not already stopped manually
            if self.input_selection.currentText() != "null":
                logger.info("🛑 Extraction stopped or completed.")
    ...
    def extract_frames_from_desktop(self):
        ...
        while self.extracting_frames:
            ...
            if not model_loaded:
                ...
                frame_filename = f"{prefix}_frame_{frame_count}{self.image_format}"
    ...
    def extract_frames_from_video(self, video_path):
        """Extract frames from the given video file and update progress."""
        ...
        if total_frames == 0:
            logger.warning("⚠️ Video has 0 frames.")
            cap.release()
            return

        self.label_progress.setValue(0)  # Reset progress bar
        ...
        frame_count = 0
        while frame_count < total_frames and self.extracting_frames:
            ret, frame = cap.read()
            if not ret:
        ...
        logger.info(f"✅ Completed frame extraction: {frame_count} frames saved.")
        self.label_progress.setValue(100)
        QMessageBox.information(self, "Info", f"Frame extraction completed.\nTotal frames saved: {frame_count}")
    ...
    def save_frame_on_inference(self, frame, results=None, frame_count=None, output_dir=None):
        try:
            ...
            if not output_dir:
                # Use self.video_name from extract logic if set, else fallback
                folder_name = getattr(self, "video_name", None)
                if not folder_name:
                    current_source = self.input_selection.currentText()
                    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                    folder_name = f"{current_source.replace(' ', '_')}_{timestamp}"
                
                output_dir = os.path.join(self.output_path, f"{folder_name}_Frames")
                os.makedirs(output_dir, exist_ok=True)
            ...
            if frame_count is not None and hasattr(self, 'skip_frames_count'):
                ...
                if frame_count % self.skip_frames_count != 0:
                    return
            # 🧠 Add unique prefix to all filenames based on session
            prefix = getattr(self, "video_name", "session")
            ...
            if frame_count is not None:
                ...
                frame_filename = f"{prefix}_frame_{timestamp}{self.image_format}"
            ...
            if not self.inference_enabled:
                cv2.imwrite(frame_filepath, frame)
                return
            # If results are not provided, perform inference
            if results is None:
                processed_frame, _ = self.apply_preprocessing(frame)
                model_kwargs = self.get_model_kwargs()
                results = self.model(processed_frame, **model_kwargs)
            # If results are empty or None, save the frame without annotations
            ...
            # Check if the result has keypoints, masks, or boxes
            ...
            if is_keypoint and has_keypoints:
                ...
                for box_idx, (box, class_id) in enumerate(zip(boxes, class_ids)):
                    ...
                    if len(keypoints) > box_idx:
                        for idx, (x, y) in enumerate(keypoints[box_idx]):
                            if idx < num_expected_keypoints:
                                nx = x / img_width
                                ny = y / img_height
                                v = 2 if (nx > 0 or ny > 0) else 0
                                final_kpt_strs[idx] = (nx, ny, v)
                    ...
                    box_obj = BoundingBox(
                        ...
                        width=w,
                        height=h,
                        keypoints=final_kpt_strs
                    )
                    labels.append(box_obj.to_str(remove_confidence=True))
            ...
            # Handle OBBs (Oriented Bounding Boxes)
            elif is_obb and hasattr(result, "obb") and result.obb:
                ...
                for obb, class_id, conf in zip(obbs, class_ids, confidences):
                    if conf < 0.35:
                        continue
                    x, y, w, h, angle = obb
                    corners = self.obb_xywhr_to_corners(x, y, w, h, angle)
            ...
            elif self.is_boxes_mode() and has_boxes:
                boxes = result.boxes.xyxy.cpu().numpy()
                class_ids = result.boxes.cls.cpu().numpy()
    ...
    def on_play_video_clicked(self):
        ...
        # 🔧 Set video_name here for saving to a folder later
        self.video_name = os.path.splitext(os.path.basename(video_path))[0]
        ...
        if self.capture and self.capture.isOpened():
            ...
            try:
                ...
                if self.model and getattr(self, 'inference_enabled', True):
                    frame_tensor = torch.from_numpy(frame.astype(np.float32)).to('cuda', non_blocking=True) / 255.0
                    ...
                    # ✅ Only save if playback extraction is allowed
                    if self.playback_extraction_enabled and results is not None:
                        self.save_frame_on_inference(
                            frame=frame,
                            results=results,
                            frame_count=self.frame_counter,
                            output_dir=os.path.join(self.output_path, f"{self.video_name}_Extracted_Frames")
    ...
    def dropEvent(self, event):
        ...
        existing_paths = {
            self.video_table.item(row, 0).text().lower()
            for row in range(self.video_table.rowCount())
        }

        for url in event.mimeData().urls():
            file_path = url.toLocalFile()
            if file_path.lower().endswith(SUPPORTED_VIDEO_EXTS) and file_path.lower() not in existing_paths:
                self.add_video_to_table(file_path)
                logger.info(f"🎞️ Video added via drop: {file_path}")
            elif file_path.lower() in existing_paths:
                logger.info(f"⚠️ Skipped duplicate video from drop: {file_path}")
    ...
    def on_add_video_clicked(self):
        ...
        existing_paths = {
            self.video_table.item(row, 0).text()
            for row in range(self.video_table.rowCount())
        }
        ...
        logger.info(f"🎞️ Video added: {video_path}")
    ...
    def resume_video_playback(self):
        if self.capture and self.capture.isOpened():
            ...
            if fps <= 0 or fps > 240:
                fps = 30
            interval_ms = int(1000 / (fps * self.playback_speed))
            self.timer2.start(interval_ms)
            logger.info(f"Playback resumed at {self.playback_speed}x speed.")
    ...
    def slider_pressed(self):
        ...
        if getattr(self, 'timer2', None) and self.timer2.isActive():
    ...
    def skip_forward(self):
        ...
        current_idx = self.speed_levels.index(self.playback_speed)
        if current_idx < len(self.speed_levels) - 1:
            self.playback_speed = self.speed_levels[current_idx + 1]
            logger.info(f"⏩ Playback speed increased to {self.playback_speed}x")
            if self.is_playing:
    ...
    def skip_backward(self):
        ...
        if current_idx > 0:
            self.playback_speed = self.speed_levels[current_idx - 1]
            logger.info(f"⏪ Playback speed decreased to {self.playback_speed}x")
            if self.is_playing:
                self.resume_video_playback()
    ...
    def process_images_cuda(self, image_files, thumbnails_directory):
        ...
        with ProcessPoolExecutor(max_workers=4) as executor:
            processed_batch = list(executor.map(load_and_process, image_files))

        return processed_batch



    def start_thumbnail_batch_worker(self):
        ...
        self.thumbnail_worker.finished.connect(self.on_thumbnail_worker_finished)
    ...
    def on_thumbnail_worker_finished(self):
        logger.info("Thumbnail batch processing completed.")
        QApplication.processEvents()


    def process_images_lazy_batches(self, image_directory, batch_size, thumbnails_directory):
        image_files = [
            f for f in glob.glob(os.path.join(image_directory, '*'))
            if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif'))
            and not self.is_placeholder_file(f)
        ]
        ...
        for batch_index, batch in enumerate(self.lazy_image_batch_generator(image_files, batch_size)):
            logger.info(f"Processing batch {batch_index + 1}/{(len(image_files) // batch_size) + 1}...")
            ...
            with ThreadPoolExecutor(max_workers=8) as executor:
                futures = []
                for image_file, pil_image in processed_batch:
                    ...
                    for idx, label_data in enumerate(labels):
                        futures.append(
                            executor.submit(
                                ...
                                image_file, pil_image, label_data, idx, thumbnails_directory
                            )
                        )
                concurrent.futures.wait(futures)

            QApplication.processEvents()  # ✅ Ensure UI stays updated incrementally
    ...
    def create_thumbnail_for_label(self, image_file, pil_image, label_data, label_index, thumbnails_directory):
        if self.is_placeholder_file(image_file):
            logger.info(f"Skipping thumbnail creation for placeholder file: {image_file}")
            return  # Explicit skip
        bbox = BoundingBox.from_str(label_data.strip())
        if bbox is None:
            logger.warning(f"Invalid label data for thumbnail: {label_data}")
            return
    ...
    def eventFilter(self, source, event):
        try:
            ...
            # 🟢 Handle key presses inside preview_list for navigation
            if event.type() == QEvent.KeyPress and source is self.preview_list:
                key = event.text()
    ...
    # 4. REPLACE your unhighlight_all_thumbnails method with this efficient version:
    def unhighlight_all_thumbnails(self):
        """Efficiently unhighlight only the currently highlighted thumbnail"""
        if self._highlighted_row is not None:
            thumbnail_label = self.preview_list.cellWidget(self._highlighted_row, 0)
    ...
    def handle_right_click(self, index):
        """Enhanced right-click handler with list locking"""
        ...
        # LOCK THE PREVIEW LIST
        self.preview_list.setEnabled(False)
        
        try:
            # Get all the data we need BEFORE any UI updates
            ...
            label_type = bbox_item.data(Qt.UserRole + 1)
            ...
            # Now do the UI updates
            if self.current_file != image_file:
            ...
            # Find and select bbox in scene
            for item in self.screen_view.scene().items():
                if (hasattr(item, 'true_line_index') and 
                    item.true_line_index == true_line_index and
                    hasattr(item, 'file_name') and 
                    item.file_name == image_file):
                    item.setSelected(True)
                    self.screen_view.scene().setFocusItem(item)
                    break
            
            # Set deletion flag
            self._deletion_in_progress = True
            
            # Perform deletion with stored data
            self.delete_item(**deletion_data)
            
        finally:
            # Always re-enable the list
    ...
    def reinitialize_bounding_boxes(self):
        ...
        # Sort by z-value to maintain layering
        all_items.sort(key=lambda x: x.zValue())
        
        # Reconfigure each item
        for item in all_items:
            # Ensure the item accepts mouse events
            ...
            # Ensure proper parent-child relationships for complex items
    ...
    def delete_item(self, row, image_file, true_line_index, label_type):
        """Enhanced delete_item that maintains UI position"""
        if self.is_placeholder_file(image_file):
            logger.info(f"Skipping deletion for placeholder file: {image_file}")
            return

        # Store current scroll position and selection
        scroll_pos = self.preview_list.verticalScrollBar().value()
        current_row = self.preview_list.currentRow()
        
        # Store the viewport position of the clicked row
        viewport_pos = self.preview_list.visualItemRect(self.preview_list.item(row, 0)).center()
        ...
        # Remove the line from the label file
        ...
        try:
            # Your existing deletion and regeneration code here...
            base_name = os.path.splitext(os.path.basename(image_file))[0]
            thumbnail_pattern = os.path.join(self.thumbnails_directory, f"{base_name}_*.jpeg")
            for old_thumb in glob.glob(thumbnail_pattern):
                try:
                    os.remove(old_thumb)
                except Exception as e:
                    logger.warning(f"Could not delete thumbnail {old_thumb}: {e}")

            # Regenerate thumbnails
            self._regenerate_thumbnails_for_image(image_file, lines)
            ...
            self.preview_list.verticalScrollBar().setValue(scroll_pos)
            
            # Try to select the next appropriate row
            if current_row < self.preview_list.rowCount():
                self.preview_list.setCurrentCell(current_row, 0)
            elif self.preview_list.rowCount() > 0:
                self.preview_list.setCurrentCell(self.preview_list.rowCount() - 1, 0)

    def _regenerate_thumbnails_for_image(self, image_file, lines):
        """Regenerate all thumbnails for an image with correct indices"""
        pixmap = QPixmap(image_file)
        img_width, img_height = pixmap.width(), pixmap.height()
        base_name = os.path.splitext(os.path.basename(image_file))[0]
        
        # Remove old thumbnails efficiently
        pattern = os.path.join(self.thumbnails_directory, f"{base_name}_*.jpeg")
        old_thumbnails = glob.glob(pattern)
        for old_thumb in old_thumbnails:
            try:
                os.remove(old_thumb)
            except Exception as e:
                logger.warning(f"Could not delete thumbnail {old_thumb}: {e}")
        
        # Find and remove all rows for this image
        rows_to_remove = []
        for i in range(self.preview_list.rowCount()):
            item = self.preview_list.item(i, 0)
            if item and item.text() == image_file:
                rows_to_remove.append(i)
        
        for i in reversed(rows_to_remove):
            self.preview_list.removeRow(i)
        
        # Regenerate thumbnails using helper
        for idx, line in enumerate(lines):
            bbox = BoundingBox.from_str(line.strip())
            if bbox:
                self._create_thumbnail_widget(image_file, bbox, idx, img_width, img_height, pixmap)
    ...
    def _create_thumbnail_widget(self, image_file, bbox, idx, img_width, img_height, pixmap):
        """Helper to create thumbnail widget and table row - reduces duplication"""
        polygon = bbox.to_polygon(img_width, img_height)
        ...
        base_name = os.path.splitext(os.path.basename(image_file))[0]
        thumbnail_filename = os.path.join(self.thumbnails_directory, f"{base_name}_{idx}.jpeg")
        if not os.path.exists(thumbnail_filename):  # Only save if needed
            resized_pixmap.save(thumbnail_filename, "JPEG")
        ...
        bbox_item = QTableWidgetItem(bbox.to_str())
        ...
        self.preview_list.setItem(row_count, 3, QTableWidgetItem(f"{x2 - x1}x{y2 - y1}"))
    ...
    def _run_debounced_thumbnail_page_now(self, image_file, page=0):
        ...
        if not hasattr(self, 'thumbnails_directory') or self.thumbnails_directory is None:
            self.thumbnails_directory = os.path.join(self.image_directory, "thumbnails")
        ...
        try:
            ...
            logger.error(f"[show_thumbnail_page] Failed to read label file: {label_file} — {e}")
            return
        ...
        for line_index, line in enumerate(lines_page, start=start):
            bbox = BoundingBox.from_str(line)
            if not bbox:
                continue
            polygon = bbox.to_polygon(img_width, img_height)
            ...
            bbox_item.setData(Qt.UserRole, line_index)
            bbox_item.setData(Qt.UserRole + 1, label_type)
            bbox_item.setData(Qt.UserRole + 2, line_index)
            ...
            thumbnail_filename = os.path.join(self.thumbnails_directory, f"{base_name}_{line_index}.jpeg")
            if not os.path.exists(thumbnail_filename):  # Only save if doesn't exist
                resized_pixmap.save(thumbnail_filename, "JPEG")
        ...
        QApplication.processEvents()
        logger.info(f"[show_thumbnail_page] Updated preview for {image_file}")
    ...
    def save_label_lines(self, label_file, lines):
        """Always write out only non-empty lines with a newline."""
        try:
    ...
    def refresh_all_thumbnails(self):
        if not self.image_directory:
            logger.warning("No image directory set for thumbnail refresh.")
            return
        ...
        self.process_images_lazy_batches(
            ...
            thumbnails_directory=self.thumbnails_directory
        )

        # NEW: After ALL processing, rebuild the preview list ONCE
        self.rebuild_preview_list_for_all_images()
        logger.info("Thumbnails refreshed.")
    def rebuild_preview_list_for_all_images(self):
        self.preview_list.clearContents()
        self.preview_list.setRowCount(0)

        # Find all images with .txt labels in the directory
        image_files = [
        ...
        for image_file in image_files:
            self.show_thumbnail_page(image_file, page=0)  # 👈 Replace old call!


    def show_thumbnail_page(self, image_file, page=0):
        """Enhanced thumbnail page display with better current file tracking"""
        # Skip thumbnail generation for placeholder images
        if self.is_placeholder_file(image_file):
            logger.info("Skipping thumbnail page for placeholder image")
            return
        
        # Store the request but also check if this is for the current file
        if hasattr(self, 'current_file') and self.current_file == image_file:
            # If showing thumbnails for current file, do it immediately
            self._run_debounced_thumbnail_page_now(image_file, page)
        else:
            # Otherwise use debounce for performance
            self._pending_thumbnail_args = (image_file, page)
            self._thumbnail_timer.start(80)


    def _run_debounced_thumbnail_page(self):
        args = self._pending_thumbnail_args
        if args is None:
            return
        ...
        # Skip if placeholder
        if self.is_placeholder_file(image_file):
            logger.info("Skipping debounced thumbnail page for placeholder")
            return

        # Double-check: only show if still requested!
        if self._pending_thumbnail_args != args:
            return

        # --- Your original show_thumbnail_page logic, indented here ---
        if getattr(self, "review_mode", False):
            return

        if image_file != self.current_file:
    ...
    def handle_left_click(self, index):
        ...
        # Display the image if it's not current
    ...
    def start_extraction_worker(self):
        ...
        thumbnails_directory = os.path.join(self.image_directory, "thumbnails")
        os.makedirs(thumbnails_directory, exist_ok=True)
        batch_size = self.batch_size_spinbox.value()

        # Prepare extraction worker and thread
        self.extraction_thread = QThread()
        self.extraction_worker = ExtractionWorker(
            image_files=image_files,
            thumbnails_directory=thumbnails_directory,
            batch_size=batch_size,
            process_images_cuda=self.process_images_cuda
        )
        ...
        self.extraction_worker.finished.connect(self.on_extraction_worker_finished)
        ...
        self.extraction_worker.progress.connect(self.label_progress.setValue)
        self.extraction_worker.error.connect(self.on_extraction_worker_error)

        self.extraction_thread.start()

    def on_extraction_worker_finished(self, results):
        self.preview_list.clearContents()
        self.preview_list.setRowCount(0)

        for image_file, thumbnail_path in results:
            if not thumbnail_path or not isinstance(thumbnail_path, (str, bytes, os.PathLike)):
                continue  # <--- SKIP None, or any non-path objects
            ...
            try:
                thumbnail_pixmap = QPixmap(thumbnail_path).scaled(128, 128, Qt.KeepAspectRatio)
            except Exception as e:
            ...
            file_item = QTableWidgetItem(image_file)
            self.preview_list.setItem(row, 0, file_item)

            thumbnail_label = QLabel()
            thumbnail_label.setPixmap(thumbnail_pixmap)
            thumbnail_label.setAlignment(Qt.AlignCenter)
            self.preview_list.setCellWidget(row, 0, thumbnail_label)

            display_name_item = QTableWidgetItem(os.path.basename(image_file))
            self.preview_list.setItem(row, 1, display_name_item)

        self.preview_list.resizeRowsToContents()
        self.preview_list.resizeColumnsToContents()
        self.label_progress.setValue(100)
        logger.info("Thumbnail extraction completed successfully.")


    def on_extraction_worker_error(self, error_message):
        QMessageBox.critical(self, "Thumbnail Extraction Error", error_message)
    ...
    def extract_and_display_data(self):
        ...
        try:
            ...
            # Only include images with .txt label files (this is just for progress bar etc)
            image_files = [
                f for f in (self.filtered_image_files if self.filtered_image_files else self.image_files)
                if not self.is_placeholder_file(f) and os.path.exists(os.path.splitext(f)[0] + '.txt')
            ]
            ...
            if not image_files:
                QMessageBox.warning(self.loader.main_window, "No Labeled Images Found", "No labeled images found. Please generate labels first.")
                return
            ...
            # 🟢 ONE AND DONE: this call does all the heavy lifting, atomic, and safe
            self.process_images_lazy_batches(
                image_directory=self.image_directory,
                batch_size=batch_size,
                thumbnails_directory=self.thumbnails_directory
            ...
            self.label_progress.setValue(len(image_files))
    ...
    def lazy_image_batch_generator(self, image_files, batch_size):
        """Yield batches of images (as lists) given the batch_size."""
    ...
    def toggle_review_mode(self):
        ...
        if self.review_mode:
            self.review_button.setText("Review Off")
            self.preview_list.setEnabled(False)  # Just disable interaction!
            logger.info("Review mode enabled: thumbnail view disabled.")
        else:
            self.review_button.setText("Review On")
            self.preview_list.setEnabled(True)
    ...
    def threshold_spinbox_changed(self):
        ...
        logging.info(
            ...
            f"NMS Threshold: {self.nms_threshold:.2f}"
        )
    ...
    def open_weights(self):
        ...
        if file_name:
            ...
            self.model_directory = os.path.dirname(file_name)  # ✅ FIXED: set model directory
    ...
    def auto_label_images2(self):
        ...
        for batch_start in range(0, total_images, batch_size):
            batch_files = self.image_files[batch_start:batch_start + batch_size]
            batch_imgs = []
            img_whs = []
            ...
            for image_file in batch_files:
                ...
                img_whs.append((img_width, img_height))

                try:
                    preprocessed_image, _ = self.apply_preprocessing(
                        self.current_display_image.copy(), None, img_width, img_height)
                    batch_imgs.append(preprocessed_image)
                except Exception as e:
                    logging.warning(f"Failed to preprocess {image_file}: {e}")
                    batch_imgs.append(None)
            ...
            try:
                with torch.amp.autocast(device_type='cuda'):
                    results = self.model.predict(batch_imgs, **self.get_model_kwargs())
            except Exception as e:
            ...
            for i, (result, image_file) in enumerate(zip(results, valid_batch_files)):
                ...
                new_bboxes = self.extract_bboxes_from_result(
                    result, img_width, img_height, image_file, None, num_expected_keypoints)
                ...
                if overwrite == QMessageBox.Yes or not label_exists:
                    ...
                    deduped_bboxes = self.remove_near_duplicate_bounding_boxes(
                        combined, iou_threshold=0.5, class_aware=True)
    ...
    def class_supports_keypoints(self, class_id):
        if not hasattr(self, "class_labels") or not self.class_labels:
            return False
        try:
            return self.class_labels[class_id].lower() == "person"  # Or load this dynamically if needed
        except IndexError:
            return False
    ...
    def get_model_kwargs(self):
        """
        Builds model inference kwargs with robust class index mapping
        to prevent invalid class_id outputs when classes.txt is a filtered subset
        of the model's training classes.
        """

        conf_threshold = self.confidence_threshold_spinbox.value()
        iou_threshold = self.nms_threshold_spinbox.value()
        batch_size = self.batch_inference.value()
        use_fp16 = getattr(self, 'fp_mode', 0) == 1
        ...
        # Load desired classes (classes.txt) and model training classes (e.g. COCO80)
        if not hasattr(self, "class_labels") or not self.class_labels:
            self.class_labels = self.load_classes()

        model_training_classes = self.load_model_classes()

        # Map desired class names to their indices in the model's original classes
        class_indices = []
        for idx, model_class in enumerate(model_training_classes):
            if model_class in self.class_labels:
                class_indices.append(idx)

        if not class_indices:
            logger.warning("⚠️ No matching classes found between classes.txt and model training classes.")
            class_indices = None  # fallback to all classes
        ...
        model_kwargs = {
            ...
            'batch': batch_size,
            'device': 0,
            'classes': class_indices,  # ensure correct indices passed to model
            'half': use_fp16,
            'agnostic_nms': True,
            'max_det': 100,
            'retina_masks': True,
        ...
        logger.info(f"✅ Model kwargs for inference: {model_kwargs}")
        return model_kwargs
    ...
    def load_model_classes(self):
        """
        Loads the full list of model training classes.
        Adjust path or logic based on your setup.
        """
        model_classes_file = os.path.join(self.model_directory, "model_classes.txt")
        if not os.path.exists(model_classes_file):
            logger.warning(f"⚠️ Model classes file not found: {model_classes_file}")
            return []
    ...
    def get_detections(self, outputs: List[np.ndarray], shape: Tuple[int, int]) -> Tuple[List[List[int]], List[float], List[int]]:
        ...
        for output in outputs:
            for detection in output:
                ...
                if confidence > self.confidence_threshold_spinbox.value():
    ...
    def plot_histogram(self):
        ...
        fig = px.histogram(df, x="Area", nbins=50, title="Distribution of Label Areas",
    ...
    def plot_bar(self):
        ...
        # Just use index for x-axis, and class name as hover
        data = {
            "Index": list(range(num_classes)),
            "Count": [class_counts.get(i, 0) for i in range(num_classes)],
            "ClassName": class_names,
            "Color": [self.qcolor_to_hex(get_color(i, num_classes)) for i in range(num_classes)]
        }
        ...
        fig = px.bar(
            ...
            color='Index',  # still needs a column for coloring
    ...
    def plot_data(self):
        ...
        df = pd.DataFrame({
            ...
            "Area": areas,
            "ClassID": class_ids,
            "ClassName": [class_names[i] for i in class_ids],
            "Color": [self.qcolor_to_hex(get_color(i, num_classes)) for i in class_ids],
            "Hover": [f"[{i}] {class_names[i]}" for i in class_ids]
        })

        fig = px.scatter(
            df, x="X", y="Y", size="Area", color="Hover",
            color_discrete_sequence=df["Color"],
            hover_name="Hover",
            hover_data={"Area": True, "X": True, "Y": True, "Color": False},
            title=f"Label Center Distribution by Class (n={len(df)})"
        )

        fig.update_layout(height=600, xaxis_title="X Center", yaxis_title="Y Center", showlegend=False)
        fig.show()
    ...
    def initialize_filter_spinbox(self):    
        ...
        """
        self.filter_class_spinbox.setMinimum(-2)  # Changed from -5 to -2
    ...
    def filter_class(self, filter_index):
        ...
        for img_file in self.image_files:
            ...
            # Handle missing label files (blanks)
            if not os.path.exists(label_file):
            ...
            # Handle empty label files (also blanks)
            if not lines:
                if filter_index == -2:  # Blanks
                    self.filtered_image_files.append(img_file)
                continue
    ...
    def update_list_view(self, image_files=None):
        ...
        # Remove Nones and missing files
        image_files = [f for f in image_files if f and os.path.exists(f)]
        ...
        for idx, image_file in enumerate(image_files):
            ...
            if self.is_placeholder_file(image_file):
                base_file = f"[Placeholder] {base_file}"
            item = QStandardItem(base_file)
            item.setFlags(item.flags() & ~Qt.ItemIsEditable)
            model.appendRow(item)

        self.List_view.setModel(model)
        self.total_images.setText(f"📸 Total Images: {len(image_files)}")
        logger.info("ListView updated successfully.")



    def on_list_view_clicked(self, index):
        # Get the index directly from the ListView to match filtered_image_files exactly
        self.current_img_index = index.row()
        self.current_file = self.filtered_image_files[self.current_img_index]

        # Update display and spinbox consistently
        self.open_image(self.current_file)
    ...
    def display_all_images(self):
        ...
        image_files = self.get_image_files(self.image_directory)

        def get_label_size(image_path):
            label_path = os.path.splitext(image_path)[0] + '.txt'
            return os.path.getsize(label_path) if os.path.exists(label_path) else 0

        sorted_images = sorted(image_files, key=get_label_size, reverse=True)

        # Insert placeholder explicitly at top of the list
        self.filtered_image_files = [placeholder_path] + sorted_images

        self.update_list_view(self.filtered_image_files)
        self.clear_annotations(force_full_reset=True)
        self.current_file = placeholder_path
        self.display_image(self.current_file)
    ...
    def get_image_files(self, directory, extensions=None):
        ...
        for f in os.listdir(directory):
            ...
            if os.path.isfile(full):
                ...
                if ext in extensions and not self.is_placeholder_file(full):
                    image_files.append(full.replace("\\", "/"))
        return image_files
    ...
    def is_placeholder_file(self, file_path):
        ...
        placeholder_path = _norm('styles/images/default.png')
        placeholder_temp_path = _norm('styles/images/default_temp.png')
        return _norm(file_path) in {placeholder_path, placeholder_temp_path}
    ...
    def open_image_video(self):
        ...
        Ask the user whether to deduplicate bounding boxes before loading.
        """
        options = QFileDialog.Options()
        options |= QFileDialog.ReadOnly
        ...
        # 🔹 Prompt user about deduplication
        reply = QMessageBox.question(
            None,
            "Deduplicate Bounding Boxes",
            "Do you want to deduplicate bounding boxes before loading?\n\nThis will remove near-duplicate boxes and clean your labels.",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No
        )

        if reply == QMessageBox.Yes:
            logger.info("🔁 User chose to deduplicate bounding boxes.")
            self.deduplicate_dataset(dir_name)
        else:
            logger.info("➡️ User skipped deduplication.")

        # Continue with loading
        self.last_image_directory = dir_name
        set_key(dotenv_path, "ULTRADARKFUSION_LAST_DIR", dir_name)
        self.saveSettings()
        ...
        # Insert placeholder image (may produce a temp copy)
        ...
        # ✅ Never pass None to create_empty_txt_and_json_files
        self.create_empty_txt_and_json_files(
            dir_name,
            processed_placeholder or placeholder_image_path
        )
    ...
    def stop_existing_workers(self):
        ...
        try:
            if hasattr(self, 'thumbnail_thread') and self.thumbnail_thread and self.thumbnail_thread.isRunning():
            ...
            if hasattr(self, 'extraction_thread') and self.extraction_thread and self.extraction_thread.isRunning():
                ...
                self.extraction_worker.finished.disconnect()
    ...
    def deduplicate_single_label_file(self, txt_file):
            ...
            try:
                ...
                bboxes.sort(key=lambda b: (b.keypoints is not None and any(kp[2] > 0 for kp in b.keypoints), float(b.confidence or 0)), reverse=True)
    ...
    def create_empty_txt_and_json_files(self, image_directory, placeholder_image_path):
        ...
        for image_file in image_files:
            if self.is_placeholder_file(image_file):
                continue  # Explicitly avoid placeholder
    ...
    def display_image(self, file_name=None, image=None, rebuild_preview=True, page=0):
        """
        Loads and displays an image, updates the annotation scene, and paginates the preview thumbnails.
        """
        # ---- 1. Handle placeholder image special case ----
        if file_name and self.is_placeholder_file(file_name):
            # Clear preview list for placeholder
            self.preview_list.clearContents()
            self.preview_list.setRowCount(0)
            logger.info("Cleared preview list for placeholder image")
            rebuild_preview = False  # Don't rebuild preview for placeholder
        
        # ---- 2. Display (and paginate) thumbnail preview table ----
            ...
            # Only show first page if needed, or if switching files
        ...
        if image is not None:
            ...
            if file_name:
                ...
                bounding_boxes, segmentations, obb_boxes = [], [], []
                if label_file and not self.is_placeholder_file(label_file):
                ...
                if not has_labels:
                    ...
                    blank_scene = QGraphicsScene(0, 0, self.image.width(), self.image.height())
                    pixmap_item = QGraphicsPixmapItem(self.image)
                    pixmap_item.setTransformationMode(Qt.SmoothTransformation)
                    blank_scene.addItem(pixmap_item)
                    self.set_screen_view_scene_and_rect(blank_scene)
                    self.display_image_with_text(blank_scene, self.image)

            self.sync_list_view_selection(file_name)

            if file_name and not self.is_placeholder_file(file_name):
                # Ensure bounding boxes are properly initialized for interaction
                self.reinitialize_bounding_boxes()
                # Store current file for synchronization
                self.current_file = file_name
                
            return QPixmap.toImage(self.image) if hasattr(self, 'image') else None
    ...
    def display_image_with_text(self, scene, pixmap):
        ...
        # Adjust these values to move the border outside the image boundaries
        border_thickness = 5

        border_item = QGraphicsRectItem(
            -border_thickness / 2,  # Shift left
            -border_thickness / 2,  # Shift up
            pixmap.width() + border_thickness,  # Expand width
            pixmap.height() + border_thickness  # Expand height
        )
        border_item.setPen(QPen(QColor(0, 255, 0, 255), border_thickness))  # Thick green border
        ...
        # Optional: If you want to keep text glow, retain this block; if not, comment it out
    ...
    def display_bounding_boxes(self, rects, file_name, segmentations=None, obbs=None):
        ...
        if segmentations:
            for index, (class_id, points) in enumerate(segmentations):
                ...
                seg_item.true_line_index = index  # ✅ Critical: Explicitly store the original line index
                seg_item.setAcceptHoverEvents(True)
                seg_item.setZValue(0.1) # Lower Z-value for segmentations
    ...
    def save_bounding_boxes(self, image_file, img_width, img_height, scene=None, remove_confidence=True, extra_labels=None, log_save=True, strict=True):
        label_file = self.get_label_file(image_file)

        if self.is_placeholder_file(image_file):
            if strict:
                logger.warning(f"⚠️ Skipping save for placeholder image: {image_file}")
    ...
    def load_labels(self, label_file, img_width, img_height):
        ...
        # Update all_frame* caches for this frame
    ...
    def save_labels_to_file(self, file_path, labels, mode='w'):
        """
        Safely writes YOLO labels to file, skipping empty or invalid lines (e.g., with NaN or inf).

        Args:
            file_path (str): Destination path for the label file.
            labels (list of str): Each line is a label string.
            mode (str): File mode: 'w' (write) or 'a' (append).
        """
        import math
        ...
        def is_valid_yolo_line(line):
            ...
            if len(parts) < 5:
                return False
            try:
                floats = [float(p) for p in parts[1:]]  # skip class_id
                return all(math.isfinite(f) for f in floats)
            except ValueError:
                return False

        try:
            valid_lines = [line.strip() for line in labels if line.strip() and is_valid_yolo_line(line)]

            if valid_lines:
                with open(file_path, mode) as file:
                    file.writelines(f"{line}\n" for line in valid_lines)
            else:
                logger.warning(f"⚠️ No valid labels to write: {file_path}")
    ...
    def navigate_frame(self, direction):
        """Enhanced navigate_frame with proper bounding box synchronization"""
        if getattr(self, "navigation_busy", False):
            logger.info("Navigation already in progress, skipping request.")
            return
        ...
        try:
            ...
            if not self.is_placeholder_file(self.current_file):
                self.save_bounding_boxes(self.current_file, self.screen_view.scene().width(), self.screen_view.scene().height())
            ...
            # CRITICAL: Reinitialize bounding boxes after display
            self.reinitialize_bounding_boxes()
    ...
    def update_gui_elements(self, format_selected):
        ...
        try:
            self.nms_true_checkbox.stateChanged.disconnect()
        except TypeError:
            pass
        self.nms_true_checkbox.stateChanged.connect(lambda: self.update_gui_elements(format_selected))
...
def run_pyqt_app():
    ...
    # Define the function here, inside run_pyqt_app, so it has access to `splash` and `app`
    def start_main_window():
        splash.close()  # close splash after delay
        window = MainWindow()  # instantiate your main window
        window.show()
