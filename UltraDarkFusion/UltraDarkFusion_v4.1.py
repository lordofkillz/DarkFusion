...
from PyQt5.QtWidgets import (QApplication, QFileDialog,
                             ...
                             QTableWidgetItem, QColorDialog, QMenu,QSplashScreen,QTableView, QVBoxLayout,QWidget,QHeaderView,QStyledItemDelegate,QStyle,QTabWidget,QStyleOptionButton,QGraphicsPolygonItem,QGraphicsEllipseItem)
...
class CustomGraphicsView(QGraphicsView):
    ...
    def mousePressEvent(self, event):
        ...
        """Handles mouse press events and sets cursor to crosshair immediately."""
        ...
        if event.button() == Qt.LeftButton:
            self.setCursor(Qt.CrossCursor)  # ✅ Immediately set crosshair cursor
            self.drawing = True
    ...
    def mouseMoveEvent(self, event):
        if self.main_window.edit_mode_active():
            super().mouseMoveEvent(event)  # ✅ FIXED
            return
...
class SegmentationDrawer(QGraphicsPolygonItem):
    ...
    def finalize(self):
        ...
        if len(self.points) >= 3:
            ...
            logging.warning("Segmentation must have at least 3 points to finalize.")
...
ui_file: Path = Path(__file__).resolve().parent / "ultradarkfusion_v4.1.ui"
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def predict_and_draw_yolo_objects(self, image, image_file_path):
        ...
        self.predictor.set_image(preprocessed_image)  # ✅ SAM sees preprocessed image!
        ...
        for yolo_line in yolo_lines:
            try:
                ...
                if results.segmentation_mask is None:
                    logging.warning(f"MediaPipe failed for bbox {x_min, y_min, x_max, y_max}")
                    continue
                ...
                if self.overwrite:
                    voc_box = pbx.convert_bbox(bbox, "voc", "yolo", (img_width, img_height))
                    adjusted_boxes.append(f"{int(class_index)} {voc_box[0]:.6f} {voc_box[1]:.6f} {voc_box[2]:.6f} {voc_box[3]:.6f}\n")

            except Exception as e:
                logging.error(f"Error processing YOLO line: {e}")

        if self.overwrite and adjusted_boxes:
            self.save_labels_to_file(yolo_file_path, [line.strip() for line in adjusted_boxes], mode="w")
        ...
        # Visualize final result if checkbox enabled
    ...
    def auto_label_images2(self):
        ...
        segmentation_mode = self.is_segmentation_mode()  # ✅ this uses the actual function# ✅ Detect segmentation mode
        ...
        for idx, image_file in enumerate(self.image_files):
            ...
            try:
                ...
                bounding_boxes = []
                segmentation_data = []
                segmentation_mode = self.is_segmentation_mode()
