...
from PyQt5.QtWidgets import (QApplication, QFileDialog,
                             ...
                             QTableWidgetItem, QColorDialog, QMenu,QSplashScreen,QTableView, QVBoxLayout,QWidget,QHeaderView,QHBoxLayout,QCheckBox,QStyledItemDelegate,QStyle,QTabWidget)
...
splash = SplashScreen("styles/gifs/darkfusion2.gif")
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def filter_bboxes_in_file(self, txt_file_path, roi, image_width, image_height, enforce_within_roi=False):
        """
        Filters bounding boxes in a YOLO `.txt` file based on an ROI using NumPy for efficiency.
        ...
        roi_x1 = x_roi
        roi_y1 = y_roi
        roi_x2 = x_roi + width_roi
        roi_y2 = y_roi + height_roi
        ...
        try:
            ...
            # Check intersections or containment with the ROI
            if enforce_within_roi:
                # Fully within ROI
                condition = (x1_bbox >= roi_x1) & (y1_bbox >= roi_y1) & (x2_bbox <= roi_x2) & (y2_bbox <= roi_y2)
            else:
                # Any overlap with ROI
                condition = (x1_bbox < roi_x2) & (x2_bbox > roi_x1) & (y1_bbox < roi_y2) & (y2_bbox > roi_y1)

            # Filter the bounding boxes
            filtered_bboxes = bboxes[condition]

            # Save filtered bounding boxes back to the file
            with open(txt_file_path, 'w') as file:
                for bbox in filtered_bboxes:
                    ...
                    file.write(f"{int(class_id)} {xc:.6f} {yc:.6f} {w:.6f} {h:.6f}\n")
    ...
    def on_stop_extract_clicked(self):
        # Stop frame extraction if active
        self.extracting_frames = False
        print("Extraction process stopped.")

        # Pause video playback instead of stopping it
    ...
    def process_video_file(self, video_path):
        ...
        while cap.isOpened():
            ...
            # Just call the same helper methods:
            frame = self.resize_frame(frame)
            frame = self.crop_frame(frame)
    ...
    def display_stats(self):
        ...
        # Add Tabs
        tab_widget.addTab(self.create_general_stats_tab(stats), "General Stats")
        tab_widget.addTab(self.create_class_counts_tab(stats), "Class Counts")
        tab_widget.addTab(self.create_bias_scores_tab(stats), "Bias Scores")
    ...
    def create_class_counts_tab(self, stats):
        ...
        model = QStandardItemModel()
        model.setHorizontalHeaderLabels(["Class", "Count"])

        # Order by classes.txt
        classes = self.load_classes()
        for class_name in classes:
            count = class_counts.get(class_name, 0)
            model.appendRow([QStandardItem(class_name), QStandardItem(str(count))])
    ...
    def create_bias_scores_tab(self, stats):
        ...
        for key, value in positional_bias.items():
            model.appendRow([QStandardItem(f"Positional ({key})"), QStandardItem(str(value))])
