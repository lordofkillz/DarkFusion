...
def setup_logger():
    ...
    # ✂️ Truncate file at startup if it's too big (e.g. > 64 KB)
    ...
    # 🌀 Rotating file handler: 16 KB max, no backups
    file_handler = RotatingFileHandler(
        filename=log_file,
        maxBytes=16 * 1024,  # 16 KB max
        backupCount=0,
        encoding="utf-8"
    )
    ...
    def paint(self, painter, option, widget=None):
        ...
        if not self.main_window.hide_label_checkbox.isChecked():
            ...
            connector_rect = QRectF(
                ...
                label_rect.bottom(),
                label_rect.width(),
                self.rect().top() - label_rect.bottom()
            )
...
class KeypointDrawer(QGraphicsItem):
    ...
    def update_points(self):
        ...
        for i, (x, y) in enumerate(self.points):
            ...
            ellipse.setPos(QPointF(x * img_width, y * img_height))

            # 🔄 Get the color from the keypoint list row
            if i < total_rows:
...
class BoundingBox:
    ...
    def from_str(label_str):
        ...
        if len(parts) < 2:
            return None  # At least class_id + something
        ...
        if len(floats) == 4:
            ...
            # Keypoints (x y v) pattern
            if len(floats) >= 7 and len(floats) % 3 == 0:
            ...
            # Segmentation pattern (no bbox, just polygon)
            elif len(floats) % 2 == 0 and len(floats) >= 6:
...
ui_file: Path = Path(__file__).resolve().parent / "ultradarkfusion_v4.2.ui"
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.load_list_button.clicked.connect(self.load_keypoint_list_from_path)
        self.show()
    ...
    def load_keypoint_list_from_path(self, file_path):
        try:
            with open(file_path, "r") as f:
                data = json.load(f)
        except Exception as e:
        ...
        for idx, kp in enumerate(data.get("keypoints", [])):
            ...
            # Connect exclusivity
            checkbox.stateChanged.connect(lambda state, row=idx: self.sync_ignore_visibility(row, source="visible"))
            ignore_checkbox.stateChanged.connect(lambda state, row=idx: self.sync_ignore_visibility(row, source="ignore"))
    ...
    def update_display(self, frame, segmentations=None):
        try:
            ...
            # Remove existing scene and recreate
            ...
            # ✅ Use the pixmap size to set the scene rect
            self.graphics_scene.setSceneRect(QRectF(pixmap.rect()))

            # ✅ Fit the actual content
    ...
    def extract_and_display_data(self):
        ...
        try:
            ...
            # Use your filtered image files directly (already handled by your filter logic)
                ...
                for i in range(0, len(image_files), batch_size):
                    ...
                    for idx, image_file in enumerate(batch_images):
                        ...
                        for j, line in enumerate(lines):
                            ...
                            norm_area = bbox_area / image_area

                            # Apply correct filtering logic for classes and sizes
                            if current_filter >= 0 and class_id != current_filter:
                                continue
                            elif current_filter == -3 and norm_area >= 0.1:
                                continue
                            elif current_filter == -4 and not (0.1 <= norm_area < 0.3):
                                continue
                            elif current_filter == -5 and norm_area < 0.3:
                                continue

                            class_name = self.id_to_class.get(class_id, "Unknown")
                            x1 = int(x_center * original_width - width / 2)
                            y1 = int(y_center * original_height - height / 2)
                            x2 = int(x_center * original_width + width / 2)
                            y2 = int(y_center * original_height + height / 2)
    ...
    def auto_label_images2(self):
        ...
        for idx, image_file in enumerate(self.image_files):
            ...
            model_kwargs = self.get_model_kwargs()
            preprocessed_image, _ = self.apply_preprocessing(image.copy(), None, img_width, img_height)
            ...
            if is_keypoint and is_pose_model:
                ...
                for box, class_id, kpts in zip(boxes, class_ids, keypoints):
                    x1, y1, x2, y2 = box
                    ...
                    for i in range(num_expected_keypoints):
                        if i < len(kpts):
                            x, y = kpts[i]
                            visible = 2 if x1 <= x <= x2 and y1 <= y <= y2 else 0
                            nx = min(max(x / img_width, 0.0), 1.0)
                            ny = min(max(y / img_height, 0.0), 1.0)
                            kpt_strs.append(f"{nx:.6f} {ny:.6f} {visible}")
                        else:
                            kpt_strs.append("0.000000 0.000000 0")
                    new_labels.append(f"{int(class_id)} {xc:.6f} {yc:.6f} {w:.6f} {h:.6f} {' '.join(kpt_strs)}")

            elif is_segmentation and has_masks:
                masks = result.masks.xy
                class_ids = result.boxes.cls.cpu().numpy() if has_boxes else [0] * len(masks)
                for mask, class_id in zip(masks, class_ids):
                    mask[:, 0] /= img_width
                    mask[:, 1] /= img_height
                    new_labels.append(f"{int(class_id)} " + " ".join([f"{p:.6f}" for p in mask.flatten()]))
                ...
                for box, class_id in zip(boxes, class_ids):
                    ...
                    w = (x2 - x1) / img_width
                    h = (y2 - y1) / img_height
                    head_label = self.generate_head_labels((x1, y1, x2, y2), img_width, img_height, int(class_id))
                    new_labels.append(f"{int(class_id)} {xc:.6f} {yc:.6f} {w:.6f} {h:.6f}")
                    if head_label:
                        new_labels.append(head_label)
            ...
            # 🔁 Inject keypoints into existing boxes if not overwriting
            # 🔄 Inject keypoints into existing bboxes if keypoint mode is active and we're not overwriting
            if is_keypoint and is_pose_model and not overwrite == QMessageBox.Yes and existing_bboxes:
                logging.info("🧠 Injecting keypoints into existing boxes")
                ...
                for eb in existing_bboxes:
                    x1 = (eb.x_center - eb.width / 2) * img_width
                    y1 = (eb.y_center - eb.height / 2) * img_height
                    x2 = (eb.x_center + eb.width / 2) * img_width
                    y2 = (eb.y_center + eb.height / 2) * img_height
                    ...
                    for i, pred_box in enumerate(boxes):
                        ...
                        pred_bbox = BoundingBox(
                            class_id=0,
                            x_center=(px1 + px2) / 2 / img_width,
                            y_center=(py1 + py2) / 2 / img_height,
                            width=(px2 - px1) / img_width,
                            height=(py2 - py1) / img_height
                        )
                        iou = self.calculate_iou(eb, pred_bbox)
                        if iou > best_iou:
                            best_iou = iou
                            best_kpts = keypoints[i]
                    ...
                    if best_iou > 0.5 and best_kpts is not None:
                        ...
                        for x, y in best_kpts:
                            if x1 <= x <= x2 and y1 <= y <= y2:
                                nx = min(max(x / img_width, 0.0), 1.0)
                                ny = min(max(y / img_height, 0.0), 1.0)
                                keypoint_list.append((nx, ny, 2))
                            else:
                                keypoint_list.append((0.0, 0.0, 0))

                        eb.keypoints = keypoint_list

                    updated_bboxes.append(eb)

                # 🔧 Save keypoints-injected boxes to label file
                with open(label_file, "w") as f:
                    for bbox in updated_bboxes:
                        f.write(bbox.to_str().strip() + "\n")

                self.deduplicate_single_label_file(label_file, mode="keypoints")
                logging.info(f"💾 Injected keypoints and updated: {label_file}")
                self.label_progress.setValue(idx + 1)
                QApplication.processEvents()
                continue  # Skip rest of the loop since we handled this case


            if overwrite == QMessageBox.Yes:
                self.save_labels_to_file(label_file, new_labels, mode="w")
            else:
                new_bboxes = [BoundingBox.from_str(l) for l in new_labels if BoundingBox.from_str(l)]
                ...
                if set(b.to_str() for b in all_bboxes) != set(b.to_str() for b in existing_bboxes):
                    with open(label_file, "w") as f:
                        for bbox in all_bboxes:
                            f.write(bbox.to_str().strip() + "\n")
                    logging.info(f"✅ Updated label file: {label_file}")
                else:
                    logging.info(f"🟰 No changes made to: {label_file}")

                dedup_mode = "keypoints" if self.is_keypoint_mode() else "boxes"
    ...
    def deduplicate_dataset(self, image_directory):
        ...
        for txt_file in txt_files:
            self.deduplicate_single_label_file(txt_file)  # 🔁 Reuse logic
    ...
    def deduplicate_single_label_file(self, txt_file, mode="boxes"):
        ...
        try:
            ...
            deduped = self.remove_near_duplicate_bounding_boxes(bboxes, mode=mode)
    ...
    def remove_near_duplicate_bounding_boxes(self, bounding_boxes, iou_threshold=0.5, class_aware=False, mode="boxes"):
        """
        Removes near-duplicate bounding boxes based on IoU with mode-aware logic.
        - In 'keypoints' mode: keep box+keypoints, remove duplicates without them.
        - In 'boxes' mode: prefer pure boxes, remove duplicates with/without keypoints.
        - In 'segmentation' mode: should be handled separately.

        :param bounding_boxes: List of BoundingBox objects
        :param iou_threshold: float
        :param class_aware: Only dedupe within class
        :param mode: One of 'boxes', 'keypoints'
        """
        ...
        for bbox_a in bounding_boxes:
            ...
            for bbox_b in unique_bounding_boxes:
                ...
                if iou > iou_threshold:
                    if mode == "keypoints":
                        # keep keypoint boxes, discard basic ones
                        if bbox_b.keypoints and not bbox_a.keypoints:
                            is_duplicate = True
                            break
                    elif mode == "boxes":
                        # discard everything duplicate regardless of keypoints
                        is_duplicate = True
                        break
        ...
        for item in scene.items():
            ...
            if isinstance(item, BoundingBoxDrawer) and hasattr(item, "keypoint_drawer") and item.keypoint_drawer:
                ...
                if actual != expected:
                    ...
                    logger.info(f"✅ Saved {actual} keypoints")
