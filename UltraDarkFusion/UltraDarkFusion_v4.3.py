...
class CustomGraphicsView(QGraphicsView):
    ...
    def mousePressEvent(self, event):
        # 🔒 Edit mode only works with segmentation
    ...
    def _handle_right_button_press(self, event):
        ...
        for item in self.scene().items(click_pos):  # <- more efficient than all items()
            # Handle SegmentationDrawer
            if isinstance(item, SegmentationDrawer):
                self._play_sound_and_remove_bbox(item)
                item.remove_self()
                break

            # Handle BoundingBoxDrawer
    ...
    def mouseMoveEvent(self, event):
        """Handles mouse movement events within the custom graphics view."""
        if self.main_window.edit_mode_active():
            # Let segmentation edit mode handle this naturally
        ...
        if self.drawing:
            # 🔧 Drawing segmentation or bounding boxes
        ...
        if self.selected_bbox and not self.drawing:
            # ✅ Prevent crashing on deleted or unlinked bounding boxes
            if sip.isdeleted(self.selected_bbox) or self.selected_bbox.scene() is None:
                ...
                # 🔥 Let Qt handle the dragging — do NOT manually call mouseMoveEvent
                # Optional: provide some feedback (e.g., change box color or highlight)
                self.selected_bbox.setPen(QPen(QColor(0, 255, 0), 2))  # Optional highlight on drag
            return

        # Default behavior if nothing else applies
        super().mouseMoveEvent(event)
    ...
    def mouseReleaseEvent(self, event):
        if self.main_window.edit_mode_active():
            super().mouseReleaseEvent(event)  # ✅ FIXED
            return
        super().mouseReleaseEvent(event)
        self.setCursor(Qt.ArrowCursor)  # Ensure cursor resets clearly
...
class KeypointDrawer(QGraphicsItem):
    ...
    def paint(self, painter, option, widget):
        ...
        # Fallback to full image if no parent (e.g., during preview init)
        bbox = self.parentItem().rect() if self.parentItem() else QRectF(0, 0, img_width, img_height)
        ...
        for i, (x, y) in enumerate(self.points):
            ...
            x_abs = bbox.x() + x * bbox.width()
            y_abs = bbox.y() + y * bbox.height()
    ...
    def update_points(self):
        # Remove previous keypoint ellipses
        ...
        # 📦 Optional fallback if no parent
        bbox = self.parentItem().rect() if self.parentItem() else QRectF(0, 0, img_width, img_height)

        for i, (x, y) in enumerate(self.points):
            ellipse = QGraphicsEllipseItem(-4, -4, 8, 8, self)
            ...
            if i < total_rows:
                ...
                logger.warning(f"⚠️ No color found for keypoint index {i}, defaulting to white.")
    ...
    def update_class_name_item(self):
        ...
        position = QPointF(last_point[0] * img_width + 6, last_point[1] * img_height - 12)
        self.class_name_item.setPos(position)
...
class BoundingBox:
    ...
    def __init__(self, class_id, x_center, y_center, width, height, confidence=None, segmentation=None, keypoints=None):
        self.class_id = class_id
    ...
    def from_str(label_str):
        ...
        # 1️⃣ Segmentation comes first
        if len(floats) % 2 == 0 and len(floats) >= 6:
            return BoundingBox(class_id, 0, 0, 0, 0, segmentation=floats)

        # 2️⃣ Then check normal bounding box
        if len(floats) == 4:
    ...
    def to_str(self, remove_confidence=False):
        ...
        if self.keypoints:
            kpt_str = ' '.join(f"{x:.6f} {y:.6f} {v:.0f}" for x, y, v in self.keypoints)
...
class SettingsDialog(QtWidgets.QDialog):
    ...
    def initUI(self):
        ...
        # === Scrollable Class Hotkeys Group ===
        ...
        for i in range(self.parent().classes_dropdown.count()):
            ...
            input_field = QtWidgets.QLineEdit(self.parent().settings.get(f'classHotkey_{className}', ''))
            input_field.setMaximumWidth(40)  # Compact for hotkey input
            label = QtWidgets.QLabel(f"{className}:")

            self.classHotkeyInputs[className] = input_field

            row = i // cols
            col = (i % cols) * 2  # label/input pairing

            scroll_layout.addWidget(label, row, col)
            scroll_layout.addWidget(input_field, row, col + 1)

        scroll_area.setWidget(scroll_content)
        main_layout.addWidget(QtWidgets.QLabel("Class Hotkeys:"))
        ...
        self.modeBoxInput = QtWidgets.QLineEdit(self.parent().settings.get('modeBox', ''))
        self.modeSegInput = QtWidgets.QLineEdit(self.parent().settings.get('modeSegmentation', ''))
        self.modeKptInput = QtWidgets.QLineEdit(self.parent().settings.get('modeKeypoint', ''))

        bottom_layout.addRow("Next Button:", self.nextButtonInput)
        bottom_layout.addRow("Previous Button:", self.previousButtonInput)
        bottom_layout.addRow("Delete Button:", self.deleteButtonInput)
        bottom_layout.addRow("Auto Label Hotkey:", self.autoLabelInput)
        bottom_layout.addRow("Mode: Boxes:", self.modeBoxInput)
        bottom_layout.addRow("Mode: Segmentation:", self.modeSegInput)
        bottom_layout.addRow("Mode: Keypoints:", self.modeKptInput)

        main_layout.addLayout(bottom_layout)

        save_btn = QtWidgets.QPushButton("Save Settings")
        save_btn.setMinimumHeight(35)
        save_btn.clicked.connect(self.saveSettings2)
        main_layout.addWidget(save_btn)

    def saveSettings2(self):
        self.parent().settings['nextButton'] = self.nextButtonInput.text()
        self.parent().settings['previousButton'] = self.previousButtonInput.text()
        self.parent().settings['deleteButton'] = self.deleteButtonInput.text()
        self.parent().settings['autoLabel'] = self.autoLabelInput.text()
        self.parent().settings['modeBox'] = self.modeBoxInput.text()
        self.parent().settings['modeSegmentation'] = self.modeSegInput.text()
        self.parent().settings['modeKeypoint'] = self.modeKptInput.text()

        for className, inputField in self.classHotkeyInputs.items():
            self.parent().settings[f'classHotkey_{className}'] = inputField.text()
...
class ScanAnnotations:
    ...
    def check_annotation_file(self, file_path, image_folder):
        ...
        for ext in self.valid_image_extensions:
            ...
            if os.path.exists(os.path.join(image_folder, potential_img_file)):
        ...
        with open(file_path, 'r') as f:
            for line in f:
                ...
                if len(tokens) != 5:
                    issues.append((line.strip(), ["Incorrect number of tokens"]))
                    continue
                ...
                try:
                    ...
                    if not self.is_class_index_valid(class_index):
                        issues.append((line.strip(), [f"Invalid class index {class_index} not in classes.txt"]))
                        continue
                    ...
                    # Add corrected annotation to the list
                    corrected_line = f"{class_index} {x_center} {y_center} {width} {height}"
                    lines_to_keep.append(corrected_line)

                except ValueError as ve:
                    issues.append((line.strip(), [f"Parsing error: {ve}"]))

        # Save the corrected annotations back to the file
        if lines_to_keep:
            self.save_labels_to_file(file_path, lines_to_keep, 'w')

        return issues, lines_to_keep, False
    ...
    def process_files(self, file_name, annotation_folder, image_folder, review_folder, statistics):
        try:
            ...
            # Just log the cleanup; don’t move files anymore
            if should_move:
                self.bad_labels += 1
                logging.info(f"Bad annotations removed from {file_name}, but valid labels kept.")
...
ui_file: Path = Path(__file__).resolve().parent / "ultradarkfusion_v4.3.ui"
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def predict_and_draw_yolo_objects(self, image, image_file_path):
        ...
        for yolo_line in yolo_lines:
            try:
                ...
                image_with_boxes, new_bbox = self.draw_mask_and_bbox(image_with_boxes, mask)
                if new_bbox == (0, 0, 0, 0):
                    continue
    ...
    def convert_bbox_to_segmentation(self, image, annotation_file):
        ...
        for yolo_line in yolo_lines:
            try:
                ...
                epsilon = 0.0001 * contour_length  # Adjust dynamically, min limit set
    ...
    def update_display(self, frame, segmentations=None):
        try:
            ...
            if (self.graphics_scene.width() < scene_rect.width() * 0.5 or
                self.graphics_scene.height() < scene_rect.height() * 0.5):
                logger.info("🔄 Replacing graphics scene for large resize.")
                self.graphics_scene = QGraphicsScene(self.screen_view)
                self.screen_view.setScene(self.graphics_scene)
                self.pixmap_item = None  # 💥 Critical line: reset pixmap item too

            # 🛡️ FIX: If pixmap_item is dead, create new one safely
    ...
    def save_frame_on_inference(self, frame, results=None, frame_count=None, output_dir=None):
        try:
            ...
            if not self.inference_enabled:
                ...
                logger.debug(f"💾 Saved frame (no labels): {frame_filepath}")
                return

            # Run inference
            if results is None:
                processed_frame, _ = self.apply_preprocessing(frame)
                model_kwargs = self.get_model_kwargs()  # Use your centralized config
                results = self.model(processed_frame, **model_kwargs)

            if results is None or not results or results[0] is None:
                logger.warning("⚠️ No YOLO detections. Saving frame without labels.")
                cv2.imwrite(frame_filepath, frame)
                return
            ...
            # 💡 Default Bounding Box Mode
                ...
                for box, class_id in zip(boxes, class_ids):
                    ...
                    w = (x2 - x1) / img_width
                    h = (y2 - y1) / img_height
                    labels.append(f"{int(class_id)} {xc:.6f} {yc:.6f} {w:.6f} {h:.6f}")
    ...
    def play_video_frame(self):
        if self.capture and self.capture.isOpened():
            ...
            try:
                # Perform inference only if model and inference enabled
                results = None
    ...
    def create_thumbnail_for_label(self, image_file, pil_image, label_data, label_index, thumbnails_directory):
        """Create and save thumbnail using BoundingBox helper."""
        try:
            ...
            if x_min >= x_max or y_min >= y_max:
                logger.warning(f"Skipping invalid crop region for {image_file}")
                return
    ...
    def handle_right_click(self, index):
        ...
        try:
            ...
            if image_item is not None and label_item is not None:
                ...
                self.delete_item(row, image_item, bbox_index, label_type)

                # ✅ After delete, rebuild thumbnails to match
                self.update_thumbnail_indices(image_file)

                # ✅ Optional: also refresh UI
                self.save_bounding_boxes(os.path.splitext(image_file)[0] + '.txt', *self.get_image_dimensions(image_file))
    ...
    def delete_item(self, row, image_item, bbox_index, label_type):
        image_file = image_item.text()  # Image file path

        self.update_label_file(image_file, bbox_index, label_type)  # Delete from txt

        self.delete_thumbnail(image_file, bbox_index)  # Delete only THAT thumbnail

        self.preview_list.removeRow(row)  # Remove from UI

        self.realign_remaining_entries()  # Update class names if needed

        self.synchronize_list_view(image_file)  # Keep listview synced
    ...
    def update_label_file(self, image_file, bbox_index_to_delete, label_type):
        label_file = os.path.splitext(image_file)[0] + '.txt'
        try:
            with open(label_file, 'r') as f:
                lines = f.readlines()
            ...
            for line in lines:
                ...
                if len(parts) > 5 and len(parts) % 2 == 1:
                    parsed.append("seg")
                elif len(parts) >= 7 and (len(parts) - 5) % 3 == 0:
                    parsed.append("kps")
                else:
                    parsed.append("bbox")

            matching_indices = [i for i, t in enumerate(parsed) if t == label_type]

            if bbox_index_to_delete >= len(matching_indices):
                logger.error(f"Invalid bbox index {bbox_index_to_delete} for {label_type}")
                return

            true_line_index = matching_indices[bbox_index_to_delete]
            lines.pop(true_line_index)

            with open(label_file, 'w') as f:
                f.writelines(lines)

            logger.info(f"Deleted {label_type} at true index {true_line_index}")
    ...
    def update_thumbnail_indices(self, image_file):
        """
        After a deletion, reindex the remaining thumbnails to match label file order.
        """
        base_filename = os.path.splitext(os.path.basename(image_file))[0]
        label_file = os.path.splitext(image_file)[0] + '.txt'
        ...
        try:
            with open(label_file, 'r') as f:
                updated_bboxes = f.readlines()

            # Clear out ALL old thumbnails first
            for filename in os.listdir(self.thumbnails_directory):
                if filename.startswith(base_filename) and filename.endswith((".jpeg", ".png")):
                    os.remove(os.path.join(self.thumbnails_directory, filename))
                    logger.info(f"Deleted old thumbnail: {filename}")

            # Recreate thumbnails matching the new label indices
            img = cv2.imread(image_file)
            if img is None:
            ...
            for new_index, line in enumerate(updated_bboxes):
                parts = line.strip().split()
                if len(parts) < 5:
                    continue
                ...
                if len(parts) > 5 and len(parts) % 2 == 1:
                    # Segmentation
                    polygon = np.array(list(map(float, parts[1:])), dtype=np.float32).reshape(-1, 2)
                    polygon[:, 0] *= img_width
                    ...
                    w *= img_width
                    h *= img_height
                    x_min = x_center * img_width - w / 2
                    y_min = y_center * img_height - h / 2
                    x_max = x_center * img_width + w
                    y_max = y_center * img_height + h
                ...
                if x_min >= x_max or y_min >= y_max:
                    logger.warning(f"Invalid crop for {image_file}")
                    continue
    ...
    def extract_and_display_data(self):
        """
        Extract bounding boxes and display thumbnails filtered by class or size (small, medium, large).
        """
        self.processing = True
        ...
        try:
            ...
            # Use filtered image files directly 
            ...
            logger.info(f"Applying filter: {current_filter}")
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                for i in range(0, len(image_files), batch_size):
                    ...
                    for idx, image_file in enumerate(batch_images):
                        ...
                        original_width, original_height = pil_images[idx].size
                        ...
                        img_width, img_height = original_width, original_height  # ✅ correct!

                        for j, line in enumerate(lines):
                            parts = line.strip().split()

                            # 🔥🔥 Detect label type (first)
                            if len(parts) > 5 and len(parts) % 2 == 1:
                                label_type = "seg"
                            elif len(parts) >= 7 and (len(parts) - 5) % 3 == 0:
                                label_type = "kps"
                            ...
                            # Now parse the bbox (once)
                            bbox = BoundingBox.from_str(line)
                            if bbox is None:
                                logger.warning(f"Skipping invalid label: {line}")
                                continue

                            # ----------- your normal bbox/seg/kps coordinate calculations --------
                            if bbox.segmentation:
                                seg_points = np.array(bbox.segmentation, dtype=np.float32).reshape(-1, 2)
                                seg_points[:, 0] *= img_width
                                seg_points[:, 1] *= img_height

                                x1 = int(np.min(seg_points[:, 0]))
                                y1 = int(np.min(seg_points[:, 1]))
                                x2 = int(np.max(seg_points[:, 0]))
                                y2 = int(np.max(seg_points[:, 1]))
                            ...
                            thumbnail_filename = os.path.join(self.thumbnails_directory, f"{os.path.basename(image_file)}_{j}.jpeg")
                            ...
                            if not self.dont_show_img_checkbox.isChecked():
                                resized_pixmap = cropped_pixmap.scaled(
                                    self._image_size_value, self._image_size_value, Qt.KeepAspectRatio, Qt.SmoothTransformation
                                )
                                ...
                                bbox_item.setData(Qt.UserRole, j)
                                bbox_item.setData(Qt.UserRole + 1, label_type)  # ✅ now safe!
    ...
    def auto_label_images2(self):
        ...
        if overwrite == QMessageBox.Cancel:
            logging.info("❌ Labeling canceled by user via cancel option.")
            return
        ...
        for idx, image_file in enumerate(self.image_files):
            ...
            try:
                ...
                logging.error(f"Preprocessing crashed: {e}")
                continue

            try:
                with torch.amp.autocast(device_type='cuda'):
                    results = self.model.predict(preprocessed_image, **model_kwargs)
                result = results[0]
            except Exception as e:
            ...
            is_segmentation = self.is_segmentation_mode()
            is_keypoint = self.is_keypoint_mode()
            is_pose_model = hasattr(result, "keypoints") and result.keypoints is not None
            has_masks = hasattr(result, "masks") and result.masks is not None
            has_boxes = hasattr(result, "boxes") and result.boxes is not None

            boxes = result.boxes.xyxy.cpu().numpy() if has_boxes else []
            confidences = result.boxes.conf.cpu().numpy() if has_boxes else []
            class_ids = result.boxes.cls.cpu().numpy() if has_boxes else []
            keypoints = result.keypoints.xy.cpu().numpy() if (is_keypoint and is_pose_model) else None

            new_bboxes = []
            num_expected_keypoints = self.keypoint_list.rowCount()
            points_file = os.path.join(os.path.dirname(image_file), "points.json")
            ...
            if is_keypoint and is_pose_model:
                for box, class_id, yolo_kpts in zip(boxes, class_ids, keypoints):
                    x1, y1, x2, y2 = box
                    xc = (x1 + x2) / 2 / img_width
                    yc = (y1 + y2) / 2 / img_height
                    w = (x2 - x1) / img_width
                    ...
                    cropped = image_bgr[int(y1):int(y2), int(x1):int(x2)]
                    final_kpts = [(0.0, 0.0, 0)] * num_expected_keypoints
                    visible_count = 0

                    for idx, (x, y) in enumerate(yolo_kpts):
                        if x1 <= x <= x2 and y1 <= y <= y2:
                            nx = min(max(x / img_width, 0.0), 1.0)
                            ny = min(max(y / img_height, 0.0), 1.0)
                            final_kpts[idx] = (nx, ny, 2)
                            visible_count += 1

                    if visible_count < num_expected_keypoints * 0.6 and cropped.size != 0:
                        ...
                        if results.pose_landmarks:
                            for row in range(self.keypoint_list.rowCount()):
                                ...
                                if landmark_enum is not None:
                                    ...
                                    x_abs = x1 + min(max(lm.x, 0.0), 1.0) * (x2 - x1)
                                    y_abs = y1 + min(max(lm.y, 0.0), 1.0) * (y2 - y1)
                                    if x1 <= x_abs <= x2 and y1 <= y_abs <= y2:
                                        visible = 2 if lm.visibility > 0.5 else 1
                                        final_kpts[row] = (
                                            min(max(x_abs / img_width, 0.0), 1.0),
                                            min(max(y_abs / img_height, 0.0), 1.0),
                                            visible
                                        )

                    bbox = BoundingBox(int(class_id), xc, yc, w, h, keypoints=final_kpts)
                    new_bboxes.append(bbox)

            elif is_segmentation and has_masks:
                for mask, class_id, conf in zip(result.masks.xy, class_ids, confidences):
                    if conf < 0.35:
                        continue
                    mask[:, 0] /= img_width
                    mask[:, 1] /= img_height
                    new_bboxes.append(BoundingBox(int(class_id), 0, 0, 0, 0, segmentation=mask.flatten().tolist()))
            ...
            if overwrite == QMessageBox.Yes or not label_exists:
                with open(label_file, 'w') as f:
                    for bbox in new_bboxes:
                        f.write(bbox.to_str().strip() + "\n")
                logging.info(f"💾 Overwritten label file: {label_file}")
            else:
                all_bboxes = existing_bboxes.copy()
                for new_bbox in new_bboxes:
                    keep = True
                    to_remove = []
                    for existing_bbox in all_bboxes:
                        iou = self.calculate_iou(new_bbox, existing_bbox)
                        if iou > 0.5:
                            if new_bbox.keypoints and not existing_bbox.keypoints:
                                to_remove.append(existing_bbox)
                            elif new_bbox.keypoints and existing_bbox.keypoints:
                                if new_bbox.num_visible_keypoints() > existing_bbox.num_visible_keypoints():
                                    to_remove.append(existing_bbox)
                                else:
                                    keep = False
                                    break
                            elif not new_bbox.keypoints and existing_bbox.keypoints:
                                keep = False
                                break
                            elif not new_bbox.keypoints and not existing_bbox.keypoints:
                                if new_bbox.confidence_safe > existing_bbox.confidence_safe:
                                    to_remove.append(existing_bbox)
                                else:
                                    keep = False
                                    break
                    for box in to_remove:
                        if box in all_bboxes:
                            all_bboxes.remove(box)
                    if keep:
                ...
                with open(label_file, 'w') as f:
                    for bbox in all_bboxes:
                        f.write(bbox.to_str().strip() + "\n")
                logging.info(f"✅ Updated label file: {label_file}")

            dedup_mode = "keypoints" if is_keypoint else "boxes"
            self.deduplicate_single_label_file(label_file, mode=dedup_mode)
    ...
    def filter_class(self, filter_index):
        ...
        for img_file in self.image_files:
            ...
            elif filter_index in [-3, -4, -5]:  # Small, Medium, Large
                try:
                    ...
                    logger.error(f"Error reading {img_file} for size filter: {e}")
    ...
    def move_filtered_images(self):
        ...
        # Prevent moving for non-class selections
        if (current_text.startswith("All") or current_text.startswith("Blanks") or
            current_text.startswith("Size:")):
            logger.warning("This function should only be used for specific classes, not 'All', 'Blanks', or size filters.")
            return
    ...
    def clear_annotations(self, force_full_reset=False):
        ...
        # Clear other cached stuff
        if hasattr(self, "all_frame_bounding_boxes"):
            self.all_frame_bounding_boxes.clear()
        if hasattr(self, "all_frame_segmentations"):
    ...
    def deduplicate_dataset(self, image_directory, max_workers=None):
        """
        Deduplicates all bounding box label files in parallel.
        Uses label_progress to update the progress bar safely.
        """
        logger.info("🔁 Deduplicating bounding boxes across the dataset (parallel mode)...")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                # Update progress bar SAFELY inside GUI thread
                if hasattr(self, 'label_progress'):
    ...
    def deduplicate_single_label_file(self, txt_file, mode="boxes"):
        ...
        try:
            ...
            for line in lines:
                bbox = BoundingBox.from_str(line)
                if bbox and not bbox.segmentation:
                    bboxes.append(bbox)
                else:
                    preserved_lines.append(line)

            deduped = self.remove_near_duplicate_bounding_boxes(bboxes, mode=mode, class_aware=True)

            deduped_lines = [b.to_str() for b in deduped]

            with open(txt_file, 'w') as f:
                f.write('\n'.join(deduped_lines + preserved_lines) + '\n')

            logger.info(f"✅ Deduplicated {txt_file} ({len(bboxes)} ➜ {len(deduped)})")

        except Exception as e:
            logger.error(f"❌ Failed to process {txt_file}: {e}")


    def remove_near_duplicate_bounding_boxes(self, bounding_boxes, iou_threshold=0.5, class_aware=False, mode="boxes", iou_mode="iou"):
        """
        Fast vectorized deduplication based on IoU.
        """
        ...
        boxes = np.array([
            [
                bbox.x_center - bbox.width / 2,  # x1
                bbox.y_center - bbox.height / 2, # y1
                bbox.x_center + bbox.width / 2,  # x2
                bbox.y_center + bbox.height / 2, # y2
                bbox.class_id
            ]
        ...
        logging.debug(f"Removed {len(bounding_boxes) - len(deduped)} duplicates (vectorized).")
        return deduped
    ...
    def calculate_iou(self, bbox_a, bbox_b, mode="iou"):
        """
        Calculate IoU, GIoU, DIoU, or CIoU between two bounding boxes.

        :param bbox_a: BoundingBox
        :param bbox_b: BoundingBox
        :param mode: 'iou', 'giou', 'diou', 'ciou'
        :return: IoU value
        """
        # Convert center coordinates to (x_min, y_min, x_max, y_max)
        xa1 = bbox_a.x_center - bbox_a.width / 2
        ya1 = bbox_a.y_center - bbox_a.height / 2
        xa2 = bbox_a.x_center + bbox_a.width / 2
        ya2 = bbox_a.y_center + bbox_a.height / 2

        xb1 = bbox_b.x_center - bbox_b.width / 2
        yb1 = bbox_b.y_center - bbox_b.height / 2
        xb2 = bbox_b.x_center + bbox_b.width / 2
        yb2 = bbox_b.y_center + bbox_b.height / 2
        ...
        if mode == "giou":
            enclose_area = c_w * c_h
            giou = iou - (enclose_area - union_area) / enclose_area
            return giou

        center_a_x = (xa1 + xa2) / 2
        center_a_y = (ya1 + ya2) / 2
        center_b_x = (xb1 + xb2) / 2
        center_b_y = (yb1 + yb2) / 2

        center_dist_sq = (center_a_x - center_b_x) ** 2 + (center_a_y - center_b_y) ** 2
        diag_sq = (c_w ** 2 + c_h ** 2)
        ...
        if mode == "ciou":
            ...
            import math
            v = (4 / (math.pi ** 2)) * (math.atan(bbox_b.width / bbox_b.height) - math.atan(bbox_a.width / bbox_a.height)) ** 2
            with torch.no_grad():
                alpha = v / (1 - iou + v + 1e-7)
            ciou = iou - (center_dist_sq / diag_sq + v * alpha)
            return ciou

        return iou  # fallback safe
    ...
    def flash_overlay_text(self, text, duration=1500, color=QColor(255, 255, 255), font_size=28):
        """
        Shows a flashing text overlay on top of the current image view with a colored glow.
        """
        scene = self.screen_view.scene()
        if not scene:
            return

        # Remove any existing overlay items with the same tag
        ...
        text_item.setZValue(100)  # Bring to front

        # Add matching glow effect (slightly darker version of text color)
        glow = QGraphicsDropShadowEffect()
        glow.setBlurRadius(25)
        glow.setColor(color.darker(150))  # Darker hue of the same class color
        glow.setOffset(3, 3)
        ...
        QTimer.singleShot(duration, lambda: scene.removeItem(text_item))
    ...
    def display_bounding_boxes(self, rects, file_name, segmentations=None):
        ...
        for index, rect_tuple in enumerate(rects):
            if len(rect_tuple) == 4:
                ...
                # 🔒 Prevent accidental keypoint creation from bad data
                is_keypoint_data = (
            ...
            # 🟡 Draw Keypoints if validated and available
            if keypoints:
        ...
        # 🔷 Display Segmentations (no bboxes!)
        if segmentations:
    ...
    def save_bounding_boxes(self, image_file, img_width, img_height, scene=None, remove_confidence=True, extra_labels=None, log_save=True):
        ...
        # 🧱 Process all items in the scene once
        for item in scene.items():

            # 🔲 Save regular bounding boxes (without keypoints)
            if isinstance(item, BoundingBoxDrawer) and not isinstance(item, SegmentationDrawer):
                if not hasattr(item, "keypoint_drawer") or not item.keypoint_drawer:
                    ...
                    bbox_lines.append(bbox.to_str(remove_confidence=remove_confidence))

            # 🟡 Save keypoints (bbox + keypoints)
            if isinstance(item, BoundingBoxDrawer) and hasattr(item, "keypoint_drawer") and item.keypoint_drawer:
                ...
                drawer = item.keypoint_drawer
                keypoint_str = drawer.get_keypoint_string()

                expected = self.keypoint_list.rowCount()
                actual = len(keypoint_str.strip().split()) // 3
                if actual != expected:
                    logger.warning(f"⚠️ Keypoint count mismatch for {os.path.basename(label_file)}: expected {expected}, saved {actual}")
                else:
                    logger.debug(f"✅ Keypoint count matches for {os.path.basename(label_file)}: {expected}")
                ...
                keypoint_lines.append(keypoint_line)

            # 🔷 Collect raw segmentations
            if isinstance(item, SegmentationDrawer) and hasattr(item, "points") and item.points:
                raw_segmentations.append((item.class_id, item.points))

        # 🧼 Deduplicate segmentations
        deduped_segmentations = self.remove_duplicate_segmentations(raw_segmentations)

        # 📝 Format segmentation labels
        segmentation_labels = [
            f"{class_id} " + " ".join(f"{x:.6f} {y:.6f}" for x, y in points)
            for class_id, points in deduped_segmentations
        ]

        # 🧼 Optional bbox deduplication
        all_bboxes = [BoundingBox.from_str(line) for line in bbox_lines]
        all_bboxes = self.remove_near_duplicate_bounding_boxes(all_bboxes)
        bbox_lines = [bbox.to_str(remove_confidence=remove_confidence) for bbox in all_bboxes]

        # 📦 Final label output
        all_labels = bbox_lines + keypoint_lines + segmentation_labels
        if extra_labels:
        ...
        if log_save:
            logger.info(f"✅ Saved {len(bbox_lines)} bboxes, {len(keypoint_lines)} keypoints, and {len(segmentation_labels)} segmentations → {label_file}")
    ...
    def load_labels(self, label_file, img_width, img_height):
        ...
        with open(label_file, 'r') as f:
            for line in f:
                try:
                    ...
                    logger.warning(f"⚠️ Non-numeric line: {line}")
                    continue
                ...
                # 🟨 SEGMENTATION: class + even number of x/y pairs
                if (
                    num_values >= 6 and
                    num_values % 2 == 0 and
                    all(0.0 <= v <= 1.0 + epsilon for v in values)
                ):
                    # Double check: not matching keypoint triplets
                    is_probably_keypoints = (
                        (num_values - 4) % 3 == 0 and
                        all(
                            i % 3 == 2 and values[i] in (0.0, 1.0, 2.0)
                ...
                if num_values == 4:
                    ...
                    if w < epsilon or h < epsilon:
                        logger.warning(f"⚠️ Zero-size bbox skipped → {label_file}")
                        continue
                    rect = self.yolo_to_rect(xc, yc, w, h, img_width, img_height)
        ...
        if label_file not in self.logged_label_files:
            logger.info(f"📄 Processed: {label_file} | 🟢 {len(bounding_boxes)} boxes, 🔷 {len(segmentations)} segmentations")
    ...
    def auto_save_bounding_boxes(self):
        """
        Prevent premature saving if keypoint drawers are not fully initialized.
        """
        if self.current_file and not self.is_placeholder_file(self.current_file):
            ...
            if not rects and not segmentations:
                return
            ...
            for rect in rects:
                ...
                if drawer:
                    # 🔒 Don't save until visibility flags are fully populated
                    if len(drawer.visibility_flags) < total_kpts:
                        #logger.info(f"⏳ Skipping auto-save: keypoint flags not fully initialized for {rect.class_id}")
                        return

            # ✅ All keypoint drawers look initialized
