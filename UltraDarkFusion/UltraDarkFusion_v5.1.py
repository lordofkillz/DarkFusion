...
class CustomGraphicsView(QGraphicsView):
    ...
    def _play_sound_and_remove_bbox(self, item):
        ...
        self.main_window.rebuild_preview_list(self.main_window.current_file, clear_table=True)
...
class ExtractionWorker(QObject):
    progress = pyqtSignal(int)
    finished = pyqtSignal(list)  # Emit results
    ...
    def is_placeholder_file(self, file_path):
        """Local check for placeholder files by basename."""
        ...
        try:
            ...
            for i in range(0, total_images, self.batch_size):
                ...
                for image_file, pil_image in processed_batch:
                    ...
                    batch_results.append((image_file, thumbnail_path))
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        ui_path = Path(__file__).resolve().parent / "UltraDarkFusion_v5.1.ui"
    ...
    def process_images_cuda(self, image_files: List[str]) -> List[Tuple[str, Optional[Image.Image]]]:
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            return list(executor.map(load_and_process, image_files))
    ...
    def lazy_image_batch_generator(self, image_files: List[str], batch_size: int):
     ...
     # Gather image files
        image_files = [
            ...
            and not self.is_placeholder_file(f)    # <--- Add this line!
        ]
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            for batch_index, batch in enumerate(batch_generator):
                ...
                processed_batch = self.process_images_cuda(batch)
    ...
    def refresh_all_thumbnails(self):
        """Rebuild all thumbnails from label files."""
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            for i in range(0, len(image_files), batch_size):
                ...
                processed = self.process_images_cuda(batch)

                for image_file, pil_image in processed:
                    if pil_image is None:
                        continue
                    if not os.path.exists(os.path.splitext(image_file)[0] + '.txt'):
                        continue
                    ...
                    for idx, line in enumerate(lines):
                        executor.submit(
                            self.create_thumbnail_for_label,
                            image_file, pil_image, line, idx, self.thumbnails_directory
                        )
                self.label_progress.setValue(self.label_progress.value() + len(batch))

        logger.info("Thumbnails refreshed.")

    def rebuild_preview_list(self, image_file, clear_table=True, specific_line_indexes: Optional[List[int]] = None):
        if not hasattr(self, 'thumbnails_directory') or self.thumbnails_directory is None:
            self.thumbnails_directory = os.path.join(self.image_directory, "thumbnails")
            os.makedirs(self.thumbnails_directory, exist_ok=True)
            logger.warning("thumbnails_directory was unset, initialized to default.")

        if self.is_placeholder_file(image_file):
            logger.info(f"Skipping rebuild for placeholder: {image_file}")
            return
        ...
        try:
            ...
            logger.error(f"[rebuild_preview_list] Failed to read label file: {label_file} — {e}")
            return

        # Delete only affected thumbnails
        base_name = os.path.splitext(os.path.basename(image_file))[0]
        if specific_line_indexes:
            for idx in specific_line_indexes:
                thumb_path = os.path.join(self.thumbnails_directory, f"{base_name}_{idx}.jpeg")
                self.attempt_delete_thumbnail(thumb_path)
        ...
        if clear_table:
            ...
            logger.info(f"[rebuild_preview_list] Cleared preview list for {image_file}")

        pixmap = QPixmap(image_file)
        img_width, img_height = pixmap.width(), pixmap.height()

        for line_index, line in enumerate(lines):
            if specific_line_indexes and line_index not in specific_line_indexes:
                continue
        ...
        QApplication.processEvents()
        logger.info(f"[rebuild_preview_list] Updated preview for {image_file}")
    ...
    def on_extraction_worker_finished(self, results):
        self.preview_list.clearContents()
        self.preview_list.setRowCount(0)  # Start with clean list
    ...
    def extract_and_display_data(self):
        ...
        try:
            ...
            # ✅ Only include images that have matching .txt label files
            image_files = [
            ...
            for i in range(0, len(image_files), batch_size):
                ...
                processed = self.process_images_cuda(batch_images)

                with ThreadPoolExecutor(max_workers=10) as executor:
                    futures = []

                    for image_file, pil_image in processed:
                        if self.is_placeholder_file(image_file) or pil_image is None:
                            continue
                        ...
                        for line_index, line in enumerate(lines):
                            ...
                            thumbnail_filename = os.path.join(
                                self.thumbnails_directory,
                                f"{os.path.splitext(os.path.basename(image_file))[0]}_{line_index}.jpeg"
                            )
                ...
                self.label_progress.setValue(self.label_progress.value() + len(batch_images))
    ...
    def process_images_in_batches(self, image_files, batch_size, thumbnails_directory):
        for i in range(0, len(image_files), batch_size):
            batch = image_files[i:i + batch_size]
            self.process_image_batch(batch, thumbnails_directory)
    ...
    def auto_label_images2(self):
        ...
        for idx, image_file in enumerate(self.image_files):
            ...
            new_bboxes = self.extract_bboxes_from_result(result, img_width, img_height, image_file, None, num_expected_keypoints)
    ...
    def update_list_view(self, image_files):
        ...
        for idx, image_file in enumerate(image_files):
            ...
            # Clearly mark the placeholder in the ListView
            if self.is_placeholder_file(image_file):
    ...
    def display_all_images(self):
        ...
        # Collect all images from the directory, excluding placeholder explicitly
        image_files = [
        ...
        # Clear annotations and ensure placeholder image displays without labels
    ...
    def display_image(self, file_name=None, image=None, rebuild_preview=True):
        ...
        if rebuild_preview and file_name and not self.is_placeholder_file(file_name):
            ...
            if current_row_count == 0:
                self.rebuild_preview_list(file_name, clear_table=True)
            else:
                first_item = self.preview_list.item(0, 0)
                if first_item is None or first_item.text() != file_name:
                    self.rebuild_preview_list(file_name, clear_table=True)
                else:
        ...
        if image is not None:
            ...
            if file_name:
                ...
                if not self.is_placeholder_file(file_name):  # ✅ Only create label file for non-placeholder images
            ...
            if file_name and not self.is_placeholder_file(file_name):
                self.rebuild_preview_list(file_name)
