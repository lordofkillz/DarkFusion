    ...
    def save_frame_on_inference(self, frame, results=None, frame_count=None, output_dir=None):
        try:
            ...
            # Handle bounding boxes
                ...
                # 🧠 Only fetch tracking IDs inside box mode
                track_ids = (
                    result.boxes.id.cpu().numpy()
                    if hasattr(result.boxes, "id") and result.boxes.id is not None
                    else None
                ...
                for idx, (box, class_id, conf) in enumerate(zip(boxes, class_ids, confidences)):
                    ...
                    # ✅ Inject tracking ID *only* when available
                    if track_ids is not None and idx < len(track_ids):
                        box_obj.tracking_id = int(track_ids[idx])
    ...
    def perform_yolo_inference(self, frame):
        ...
        try:
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                if getattr(self, 'tracking_enabled', False) and self.is_boxes_mode():
