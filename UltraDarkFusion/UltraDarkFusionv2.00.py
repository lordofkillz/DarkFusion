...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def handle_left_click(self, index):
        row = index.row()
        print(f"Row clicked: {row}")  # Debugging output for the clicked row

        # Retrieve the image file path
        image_item = self.preview_list.item(row, 0)
        if not image_item:
            print("Failed to fetch image file from the list.")
            return
        image_file = image_item.text()
        print(f"Image file fetched: {image_file}")
        ...
        if self.hide_labels:
            ...
            print("Labels were hidden, now shown.")

        # Handle multiple bounding boxes for the clicked thumbnail
        self.flash_bounding_boxes_for_image(row, image_file)

        # Synchronize ListView selection
        self.synchronize_list_view(image_file)

    def flash_bounding_boxes_for_image(self, row, image_file):
        bounding_boxes = self.retrieve_bounding_boxes(row)
        if not bounding_boxes:
            print("No bounding boxes found for this row.")
            return

        for bbox_index in bounding_boxes:
            self.flash_bounding_box(bbox_index, image_file)

    def retrieve_bounding_boxes(self, row):
        # Assuming each row could have multiple bounding boxes stored
        # This function should retrieve all bounding box indices for a given row
        # For simplicity, let's assume it retrieves a list of indices
        return [self.preview_list.item(row, 4).data(Qt.UserRole)]  # Modify as needed
    ...
    def flash_bounding_box(self, bbox_index, image_file):
        ...
        for rect_item in self.screen_view.scene().items():
            ...
            if isinstance(rect_item, BoundingBoxDrawer) and rect_item.unique_id == unique_id:
                ...
                found = True
                print(f"Flashing color set and duration started for {unique_id}.")
                break
        if not found:
            print(f"No matching bounding box drawer found for {unique_id}.")
    ...
    def flash_bounding_box(self, bbox_index, image_file):
        """Flash the bounding box on the screen."""
        found = False
        for rect_item in self.screen_view.scene().items():
            if isinstance(rect_item, BoundingBoxDrawer) and rect_item.unique_id == f"{image_file}_{bbox_index}":
                rect_item.flash_color = QColor(*self.flash_color_rgb)
                rect_item.flash(duration=self.flash_time.value())
                found = True
                print(f"Flashing color set and duration started for {image_file}_{bbox_index}.")
                break
        if not found:
            print(f"No matching bounding box drawer found for {image_file}_{bbox_index}.")
