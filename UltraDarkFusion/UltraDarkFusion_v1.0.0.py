...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.stop_label.clicked.connect(self.stop_auto_labeling)
        self.image_list = []
        self.class_to_id = {}
    ...
    def all_objects_labeled(self, current_boxes, label_file):
        with open(label_file, 'r') as f:
            lines = f.readlines()

        # Remove the existing bounding boxes
        labeled_boxes = []
        for line in lines:
            components = line.strip().split()
            class_id = int(components[0])
            x_center, y_center, width, height = map(float, components[1:])
            labeled_boxes.append((class_id, x_center, y_center, width, height))


    def auto_label_images2(self):
        """
        This function automatically labels images based on a trained YOLOv8 and yolov5 (old) model.
        It reads the images from a directory, applies the YOLO model for object detection,
        pose estimation, or segmentation, and then saves the results in a text file.
        """
        ...
        # Ask the user if they want to overwrite existing labels
        ...
        for idx, image_file in enumerate(self.image_files):
            ...
            try:
                if self.model_type == 'yolov5':
                    results = self.model(self.current_file)
                    # Assuming YOLOv5 model returns results in this format
                elif self.model_type == 'yolov8':
                    results = self.model(self.current_file, classes=class_indices)
                    # Assuming YOLOv8 model requires class indices
                else:
                    raise ValueError("Unknown model type. Unable to process predictions.")
            except Exception as e:
                print(f"Error: {e}")
                return
            ...
            try:
                if 'pose' in self.weights_file_path:
                    # Placeholder, adapt as needed based on your model's output
                    keypoints = results.keypoints
                    self.display_image(image_file, keypoints=keypoints)
                elif 'seg' in self.weights_file_path:
                    # Placeholder, adapt as needed based on your model's output
                    masks = results.masks
                    for mask in masks:  # Assuming masks is a list of 2D numpy arrays
            ...
            if self.all_objects_labeled(labeled_boxes, label_file) or overwrite == QMessageBox.Yes or label_exists is False:
    ...
    def overlay_mask_on_image(image, mask):
        # Assumption: 'mask' is a binary 2D numpy array of the same HxW dimensions as the image
        color_mask = np.zeros((mask.shape[0], mask.shape[1], 3), dtype=np.uint8)
        color_mask[mask == 1] = [0, 255, 0]  # Setting the color to Green for the mask
        cv2.addWeighted(color_mask, 0.5, image, 0.5, 0, image)
...
# for blanks
    ...
    def download_videos(self):
        ...
        for row in range(self.video_download.rowCount()):
            url = self.video_download.item(row, 0).text()
            if url:  # Only process rows with a URL entered
                ...
                download_thread.update_progress.connect(
    ...
    def open_weights(self):
        ...
        file_name, _ = QFileDialog.getOpenFileName(
            None, "Open Weights/Model", "", "Weights/Model Files (*.weights *.pt)", options=options)
        if file_name:
            if hasattr(self, 'model'):
                del self.model
                self.model = None
                self.model_type = None  # Add a new attribute to store the model type
            ...
            if file_name.endswith('.pt'):
                try:
                    self.model = YOLO(file_name)  # Attempt to load with YOLOv8 first
                    self.model_type = 'yolov8'
                    print("Loaded model as YOLOv8.")
                except Exception as e_v8:
                    try:
                        self.model = yolov5.load(file_name)  # If fails, try to load with YOLOv5
                        self.model_type = 'yolov5'
                        print("Loaded model as YOLOv5.")
                    except Exception as e_v5:
                        print(f"Failed to load model. YOLOv8 error: {e_v8}. YOLOv5 error: {e_v5}")
                        self.model = None
    ...
    def auto_label_yolo_button_clicked(self) -> bool:
        ...
        if self.weights_file_path.endswith('.pt'):
    ...
    def delete_current_image(self):
        # Check if any image has been loaded
        if self.current_file is None:
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            msg.setText(
                "No image currently loaded. Please load an image before attempting to delete.")
            msg.exec_()
            return
...
# adjust and view labels for a single image
    ...
    def ultralytics_train_clicked(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "No PT File Selected", "Please select a PT file before proceeding.")
            return
        ...
        command = ("yolo task={} mode=train model={} data={} imgsz={} epochs={} "
                   "batch={} project={} save_period=-1 half=True amp=True").format(
            task, model_input, data_input, imgsz_input, epochs_input, batch_input, self.runs_directory)
