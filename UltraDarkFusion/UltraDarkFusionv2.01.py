...
from PyQt5.QtWidgets import (QApplication, QFileDialog,
                             ...
                             QTableWidgetItem, QColorDialog, QMenu,QSplashScreen,QTableView, QVBoxLayout,QWidget,QHeaderView,QHBoxLayout,QCheckBox,QStyledItemDelegate,QStyle)
...
class FileChangeHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        if event.src_path.endswith('.txt') and event.src_path.startswith(self.directory):
            # Trigger the callback when a .txt file in the directory is modified
            self.update_json_callback(event.src_path)
...
class CustomGraphicsView(QGraphicsView):
    ...
    def wheelEvent(self, event):
        ...
        # Ensure smooth scaling with floating point precision
...
class BoundingBoxDrawer(QGraphicsRectItem):
    ...
    def __init__(self, x, y, width, height, main_window, class_id=None, confidence=None, unique_id=None):
        ...
        self.setPen(QPen(self.get_color(self.class_id, main_window.classes_dropdown.count()), 1.9))
        self.setZValue(0.5)
    ...
    def paint(self, painter, option, widget=None):
        ...
        if not self.main_window.hide_label_checkbox.isChecked():
            ...
            mapped_label_shade_value = int((label_shade_value / 100) * 255)
            shaded_label_color = QColor(label_color.red(), label_color.green(), label_color.blue(), mapped_label_shade_value)
    ...
    def update_position_and_size(self):
        ...
        if self.scene() is not None:
            self.bbox.x_center = (rect.x() + rect.width() / 2) / self.scene().width()
            self.bbox.y_center = (rect.y() + rect.height() / 2) / self.scene().height()
            self.bbox.width = rect.width() / self.scene().width()
            self.bbox.height = rect.height() / self.scene().height()
...
class ScanAnnotations:
        ...
        if reply2 == QMessageBox.Yes:
            ...
            for file_name in txt_files:
                ...
                if label_size in subfolder_names:
                    ...
                    img_file = file_name.replace(".txt", ".jpg", ".jpeg", ".png")
                    img_path = os.path.join(image_folder, img_file)
    ...
    def scan_annotations(self, progress_bar):
        ...
        for img_file in img_files_set:
            ...
            # Replace the image extension with '.txt'
            if img_file.endswith(".jpg"):
                ...
                txt_file = None  # Handle cases where the file extension is not recognized

            if txt_file:
                txt_file_path = os.path.join(annotation_folder, txt_file)
                if os.path.exists(txt_file_path):
                    self.process_image_for_metadata_removal(image_path)  # No need for review folder or output path


        # Finding text files without corresponding images
        txt_files_without_img = txt_files_set - {os.path.splitext(img_file)[0] + ".txt" for img_file in img_files_set}

        # Finding images without corresponding text files
        img_files_without_txt = img_files_set - {
            file.replace(".txt", os.path.splitext(file)[1]) for file in txt_files_set
}


        for file_name in txt_files_without_img:
            src_path = Path(annotation_folder) / file_name
            dst_path = Path(review_folder) / file_name
            logging.warning(f"{file_name} does not have a corresponding image file. Moving it to the 'review' folder.")
            try:
        ...
        for img_file in img_files_without_txt:
            txt_file_path = os.path.join(annotation_folder, img_file.rsplit(".", 1)[0] + ".txt")
            if not os.path.exists(txt_file_path):
                logging.warning(f"{img_file} does not have a corresponding annotation file. Moving it to the 'review' folder.")
                shutil.move(os.path.join(image_folder, img_file), os.path.join(review_folder, img_file))
    ...
    def move_files(self, src_folder, dst_folder, file_name):
    ...
    def scan_annotations(self, progress_bar):
        ...
        txt_files_without_img = txt_files_set - {os.path.splitext(img_file)[0] + ".txt" for img_file in img_files_set}
        img_files_without_txt = img_files_set - {file.replace(".txt", os.path.splitext(file)[1]) for file in txt_files_set}
...
class DownloadThread(QThread):
    ...
    def run(self):
        ...
        # Set yt-dlp options to download video only (no audio)
        ydl_opts = {
            'format': 'bestvideo',  # Download only the best video format (no audio)
            'outtmpl': f'{self.directory}/%(title)s.%(ext)s',  # Output template for filenames
            'progress_hooks': [self.yt_dlp_progress_hook],  # Hook for progress updates
            'noplaylist': True,  # Ensure we are only downloading a single video
        }
        ...
        for attempt in range(retries):
            try:
                with YoutubeDL(ydl_opts) as ydl:
                    ...
                    self.update_status.emit(self.row, "Download complete")
                break

            except Exception as e:
                if attempt < retries - 1:
                    time.sleep(random.uniform(1, 3))  # Retry logic
                else:
...
class ImageProcessingThread(QThread):
    ...
    def fill_cropped_area(self, original_img, img, mask):
        prompt = "A photo of a tree or a bush"
        try:
            mask_image = Image.fromarray(mask).convert("RGB")
            image = Image.fromarray(cv2.cvtColor(original_img, cv2.COLOR_BGR2RGB))
            result = self.stable_diffusion_pipeline(prompt=prompt, image=image, mask_image=mask_image)
...
class RedBoxDelegate(QStyledItemDelegate):
    def paint(self, painter, option, index):
        ...
        # Check if the item is selected (correct reference to QStyle.State_Selected)
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def move_to_review_folder(self, file_path, review_folder):
        """Move bad files (both annotation and image) to a review folder for manual inspection."""
        os.makedirs(review_folder, exist_ok=True)

        file_name = os.path.basename(file_path)
        new_txt_path = os.path.join(review_folder, file_name)

        if not os.path.exists(file_path):
            logging.error(f"File {file_path} not found. Cannot move to review folder.")
            return
        ...
        for ext in image_extensions:
            potential_image_path = os.path.join(os.path.dirname(file_path), base_name + ext)
        ...
        try:
            ...
            logging.info(f"Moved annotation file {file_name} to {review_folder}")
        except Exception as e:
            logging.error(f"Failed to move annotation file {file_name} to {review_folder}: {e}")

        if image_file:
            new_image_path = os.path.join(review_folder, os.path.basename(image_file))
            try:
                shutil.move(image_file, new_image_path)
                logging.info(f"Moved image file {os.path.basename(image_file)} to {review_folder}")
            except Exception as e:
                logging.error(f"Failed to move image file {os.path.basename(image_file)} to {review_folder}: {e}")
    ...
    def copy_and_rename_files(self, image_path, txt_path, target_directory):
        ...
        logging.info(f"Copied and renamed {image_path} and {txt_path} to {target_directory}")
    ...
    def predict_and_draw_yolo_objects(self, image, image_file_path):
        ...
        Also performs segmentation using SAM, creates thumbnails, and generates a single shadow image.
        """
        ...
        full_mask = np.zeros((img_height, img_width), dtype=bool)  # Full image-sized mask to capture all objects

        segmented_folder = os.path.join(os.path.dirname(image_file_path), "segmented")
        os.makedirs(segmented_folder, exist_ok=True)

        # Processing each object in the YOLO file
        for yolo_line in yolo_lines:
            try:
                values = list(map(float, yolo_line.strip().split()))
            except ValueError as e:
                logging.error(f"Could not convert line to floats in YOLO file {yolo_file_path}: {yolo_line}")
                self.move_to_review_folder(yolo_file_path, "review")
                return None

            if len(values) != 5:
                logging.error(f"Unexpected format in YOLO file {yolo_file_path}: {yolo_line}")
                continue
            ...
            try:
                ...
                # Accumulate the mask for shadows later
                full_mask |= mask

                # Correct the bounding box and mask
                masked_image, (x_top_left, y_top_left, box_width, box_height) = self.draw_mask_and_bbox(
                    image, mask, self.input_points, min_contour_area=50
                )

                # **Always keep bounding boxes**
                if self.overwrite:
                    try:
                        # Convert VOC bbox back to YOLO format for adjusted bounding box
                        voc_box = pbx.convert_bbox(
                            (x_top_left, y_top_left, x_top_left + box_width, y_top_left + box_height),
                            from_type="voc",
                            to_type="yolo",
                            image_size=(img_width, img_height)
                        )
                        s = f"{int(class_index)} {voc_box[0]:.6f} {voc_box[1]:.6f} {voc_box[2]:.6f} {voc_box[3]:.6f}\n"
                        adjusted_boxes.append(s)
                    except Exception as e:
                        logging.error(f'Error during bounding box conversion: {e}')
                        continue

                # Create the segmented thumbnail **after** correcting the bounding box
                cropped_image = image_copy[y_top_left:y_top_left + box_height, x_top_left:x_top_left + box_width]
                mask_cropped = mask[y_top_left:y_top_left + box_height, x_top_left:x_top_left + box_width]

                segmented_thumbnail = np.zeros_like(cropped_image)
                for c in range(3):
                    segmented_thumbnail[..., c] = np.where(
                        mask_cropped,
                        cropped_image[..., c],
                        0
                    )

                # Only create thumbnail if enough of the object is segmented
                if np.count_nonzero(segmented_thumbnail) > (segmented_thumbnail.size // 10):
                ...
                # Update the original image with masked image
                image = masked_image

            except Exception as e:
                logging.error(f'Error during prediction or bounding box conversion for file {image_file_path}: {e}')
                self.move_to_review_folder(image_file_path, "review")
                return None

        # Overwrite the YOLO file with adjusted boxes, if necessary
        ...
        # Generate a single shadow image for all objects after processing all YOLO lines
        if self.shadow:
            shadow_image = image_copy.copy()
            shadow_image[full_mask] = [0, 0, 0]  # Black out the masked areas

            shadow_folder = os.path.join(os.path.dirname(image_file_path), "shadow")
            os.makedirs(shadow_folder, exist_ok=True)
            new_image_path, new_txt_path = self.copy_and_rename_files(image_file_path, yolo_file_path, shadow_folder)
            cv2.imwrite(new_image_path, shadow_image)

        return image if self.screen_update.isChecked() else None


    def draw_mask_and_bbox(self, image, mask, input_points, min_contour_area=50):
        """
        Draw bounding box around the object and apply filtering to remove small or irrelevant contours.
        """
        contours, _ = cv2.findContours(mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        # **Ensure bounding boxes are kept** even if contours are small.
        if not contours:
            logging.warning("No significant contours found in the mask. Keeping original bounding box.")
        ...
        # If the bounding box is invalid, return the full image box.
        if x_max <= x_min or y_max <= y_min:
            logging.warning("Invalid bounding box dimensions found, returning the full image as bounding box.")
            return image, (0, 0, img_width, img_height)

        # Apply mask overlay with transparency
        alpha = 0.2
        overlay = image.copy()
        cv2.fillPoly(overlay, contours, (255, 0, 0))  # Fill the object with red (or any other color)
    ...
    def update_console_output(self):
        ...
        # Update the text with more readable formatting in the PyQt widget
        init_msg = (
            ...
            f"Happy Labeling! Remember to check my <a href='https://github.com/lordofkillz/DarkFusion'>GitHub</a> regularly for updates.<br>"
        )

        text = (
            f'<font color="red">'
            f'UltraDarkLabel Initializing at: {start_time.strftime("%Y-%m-%d %H:%M:%S")}<br>'
            f'Running Time: {elapsed_time_str}<br>'
            f'CPU Usage: {cpu_usage}%<br>'
            f'GPU Usage: {gpu_usage}%<br>'
            f'Memory Usage: {memory_usage}%<br>'
            f'{init_msg}'  # Display init_msg here in the UI ONLY
            f'</font>'
        )
        ...
        # Ensure rich text and clickable links in QLabel
    ...
    def update_download_status(self, row, status):
        # Update the status in the table (column 1 or whichever column you prefer for status)
        self.video_download.setItem(row, 1, QTableWidgetItem(status))
    ...
    def onGifChange(self, index):
        if index != -1:
            ...
            self.movie.start()
    def image_batch_generator(self,image_files, batch_size):
        for i in range(0, len(image_files), batch_size):
            yield image_files[i:i+batch_size]

    def batch_process(self):
        """Process images in batch to handle large datasets more efficiently."""
        if self.image_directory is None:
            QMessageBox.warning(self, "No Directory Selected", "Please select a directory before processing.")
            return
        ...
        all_files = glob.glob(os.path.join(self.image_directory, '*'))
        image_files = [f for f in all_files if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif'))]
        
        if not image_files:
            QMessageBox.warning(self, "No Images Found", "No images found in the directory.")
            return

        batch_size = self.batch_size_spinbox.value()
        batch_generator = self.image_batch_generator(image_files, batch_size)

        # Increase parallelism by experimenting with a higher number of threads
        num_cores = multiprocessing.cpu_count()
        with ThreadPoolExecutor(max_workers=num_cores * 2) as executor:
            for batch in batch_generator:
                executor.submit(self.process_image_batch, batch)

        print(f"Processing completed for {len(image_files)} images.")

    def process_image_batch(self, batch_images):
        """Process each batch of images and save thumbnails."""
        if not self.processing:
            return  # Stop processing if flagged

        # Use more efficient image transformations
        ...
        with ThreadPoolExecutor() as io_executor:
            for image_file in batch_images:
                try:
                    # Load and process the image using OpenCV for faster performance
                    ...
                    io_executor.submit(self.create_thumbnail, image_file, transformed_img)
                except Exception as e:
                    print(f"Error processing {image_file}: {str(e)}")

        QApplication.processEvents()

    def create_thumbnail(self, image_file, transformed_img):
        ...
        thumbnail_filename = os.path.join(self.thumbnails_directory, f"{base_file}.{extension}")
    ...
    def set_image_directory(self, new_directory):
        """Update the image directory and refresh statistics."""
        ...
        self.show_labeled_stats()  # Refresh stats for the new directory
        self.display_stats()       # Refresh the table view with new statistics
    ...
    def analyze_image_quality(self, image_path):
        """Analyze blur, brightness, and contrast of an image."""
    ...
    def show_labeled_stats(self):
        """Calculate and store statistics for the current directory, including positional and size bias."""
        ...
        total_blur, total_brightness, total_contrast = 0, 0, 0
        image_qualities = []
                ...
                if annotations:
                    ...
                    for line in annotations:
                        ...
                        if len(parts) == 5:
                            ...
                            if center_x < 0.33:  # Left
                                pos_counts['left'] += 1
                            elif center_x > 0.66:  # Right
                                pos_counts['right'] += 1
                            else:  # Center
                                pos_counts['center'] += 1
                            ...
                            if center_y < 0.33:  # Top
                                pos_counts['top'] += 1
                            elif center_y > 0.66:  # Bottom
                                pos_counts['bottom'] += 1
                            else:  # Middle
                                pos_counts['middle'] += 1

                            # Track size bias (classifying sizes as small, medium, or large based on area)
                            bbox_area = width * height
                            if bbox_area < 0.1:  # Small
                                size_counts['small'] += 1
                            elif bbox_area < 0.3:  # Medium
                    ...
                    blur_value, brightness, contrast = self.analyze_image_quality(image_path)
                    if blur_value is not None:
                        image_qualities.append((blur_value, brightness, contrast))
                        total_blur += blur_value
                        total_brightness += brightness
                        total_contrast += contrast

        avg_blur = total_blur / labeled_images if labeled_images else None
        avg_brightness = total_brightness / labeled_images if labeled_images else None
        avg_contrast = total_contrast / labeled_images if labeled_images else None
        ...
        stats = {
            ...
            'Average Blur': round(avg_blur, 2) if avg_blur is not None else "N/A",
            'Average Brightness': round(avg_brightness, 2) if avg_brightness is not None else "N/A",
            'Average Contrast': round(avg_contrast, 2) if avg_contrast is not None else "N/A",
            'Positional Bias (center)': pos_counts.get('center', 0),
            'Positional Bias (left)': pos_counts.get('left', 0),
            'Positional Bias (right)': pos_counts.get('right', 0),
            'Positional Bias (top)': pos_counts.get('top', 0),
            'Positional Bias (bottom)': pos_counts.get('bottom', 0),
            'Size Bias (small)': size_counts.get('small', 0),
            'Size Bias (medium)': size_counts.get('medium', 0),
            'Size Bias (large)': size_counts.get('large', 0),
        }

        stats['Bias Score (%)'] = self.calculate_bias_score(label_counts, pos_counts, size_counts)

        for class_name in class_names:
            stats[f'{class_name} Label Count'] = label_counts[class_name]

        # Update the stats in settings
    ...
    def calculate_bias_score(self, label_counts, pos_counts, size_counts):
        ...
        # Combine the bias metrics
        combined_bias_score = (gini_coefficient + positional_bias + size_bias) / 3
        return round(combined_bias_score * 100, 2)  # Return as a percentage for easier interpretation
    ...
    def display_stats(self):
        ...
        # Clear the old table widget before updating
        ...
        # Add general statistics to the table
        general_stats_keys = [
            'Total Images', 'Labeled Images', 'Unlabeled Images',
            'Total Labels', 'Labels per Image (average)',
            'Labeling Progress (%)', 'Average Blur', 'Average Brightness', 
            'Average Contrast', 'Bias Score (%)'
        ]
        ...
        # Add class-specific label counts
        ...
        # Add positional bias metrics to the table
        positional_bias_keys = [
            'Positional Bias (center)', 'Positional Bias (left)', 'Positional Bias (right)',
            'Positional Bias (top)', 'Positional Bias (bottom)'
        ]
        
        for key in positional_bias_keys:
            item = QStandardItem(key)
            value = QStandardItem(str(stats.get(key, "N/A")))
            model.appendRow([item, value])

        # Add size bias metrics to the table
        size_bias_keys = ['Size Bias (small)', 'Size Bias (medium)', 'Size Bias (large)']
        
        for key in size_bias_keys:
            item = QStandardItem(key)
            value = QStandardItem(str(stats.get(key, "N/A")))
            model.appendRow([item, value])
    ...
    def move_filtered_images(self, filter_input):
        ...
        protected_file = 'styles/darkfusion2.png'  # Define the protected file here
    ...
    def filter_class(self, filter_input):
        ...
        placeholder_file = 'styles/darkfusion2.png'
        ...
        if placeholder_file not in filtered_image_files:
            filtered_image_files.insert(0, placeholder_file)
    ...
    def display_all_images(self):
        ...
        placeholder_path = 'styles/darkfusion2.png'  # Adjust path as needed
    ...
    def get_label_size(image_path):
        protected_file = 'styles/darkfusion2.png'  # Define the protected file here
        if image_path == protected_file:
            return -1  # Return a special value for the placeholder so it doesn't affect sorting

        label_path = os.path.splitext(image_path)[0] + '.txt'
        if os.path.exists(label_path):
            return os.path.getsize(label_path)
        else:
            # Return a special value for images without labels
            return 0  # Smaller than any possible label size
    ...
    def open_image_video(self):
        ...
        placeholder_image_path = 'styles/darkfusion2.png'
        if dir_name:
            ...
            if total_images > 0:
                ...
                threading.Thread(target=self.show_labeled_stats, daemon=True).start()
    ...
    def create_empty_txt_and_json_files(self, image_directory, placeholder_image_path):
        ...
        for image_file in image_files:
            if image_file != placeholder_image_path:  # Skip placeholder image
    ...
    def delete_current_image(self):
        ...
        placeholder_path = 'styles/darkfusion2.png'
    ...
    def delete_files(self, file_path):
        ...
        protected_file = 'styles/darkfusion2.png'
    ...
    def clear_class_boxes(self, filter_input):
        ...
        placeholder_file = 'styles/darkfusion2.png'  # Placeholder file path
    ...
    def apply_super_resolution(self, file_name):
        ...
        if self.super_resolution_Checkbox.isChecked() and file_name is not None:
            ...
            path_to_model = r"C:\EAL\Scripts\UltraDarkFusion\Sam\FSRCNN_x4.pb"
    ...
    def update_fit_and_zoom(self):
        """
        Update the fitInView_scale and zoom_scale for the new image.
        """
        self.screen_view.fitInView_scale = self.screen_view.transform().mapRect(QRectF(0, 0, 1, 1)).width()
    ...
    # Refactor next_frame and previous_frame to use the common method
    def next_frame(self):
        # Stop scanning
