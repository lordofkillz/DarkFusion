...
from PyQt5.QtGui import (QBrush, QColor, QFont, QImage, QImageReader,
                         ...
                         QStandardItemModel, QTransform, QLinearGradient,QIcon,QCursor,QStandardItemModel, QStandardItem)
from PyQt5.QtWidgets import (QApplication, QFileDialog,
                             QGraphicsDropShadowEffect, QGraphicsItem,
                             QGraphicsPixmapItem, QGraphicsRectItem,
                             QGraphicsScene, QGraphicsTextItem, QGraphicsView,
                             QLabel, QMessageBox, QProgressBar,
                             QTableWidgetItem, QColorDialog, QMenu,QSplashScreen,QTableView, QVBoxLayout,QWidget,QHeaderView,QHBoxLayout,QCheckBox,)
        ...
        for img_file in img_files_set:
            ...
            if txt_file:
                ...
                if os.path.exists(txt_file_path):
                    metadata_output_path = os.path.join(review_folder, img_file)
                    self.process_image_for_metadata_removal(image_path, metadata_output_path)
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def download_videos(self):
        ...
        for row in range(self.video_download.rowCount()):
            ...
            if item and item.text():  # Check if the item and its text are not None
                url = item.text()
    ...
    def display_camera_input(self):
        try:
            ...
            if self.crop_images_checkbox.isChecked():
                ...
                start_x = max(0, frame_width // 2 - crop_width // 2)
                start_y = max(0, frame_height // 2 - crop_height // 2)
        ...
        for row in range(self.preview_list.rowCount()):
            ...
            if thumbnail_label:
                ...
                if pixmap:
                    ...
                    thumbnail_label.update()  # Ensure the label is updated with the new pixmap
        self.preview_list.viewport().update()  # Update the entire view to reflect changes
                ...
                for idx, (image_file, original_size) in enumerate(zip(batch_images, original_sizes)):
                    ...
                    for j, line in enumerate(lines):
                        ...
                        if not self.dont_show_img_checkbox.isChecked():
                            # Resize cropped pixmap for display in the UI, without changing the saved image size
                            resized_pixmap = cropped_pixmap.scaled(
                                128, 128,  # Fit within 128x128 pixels
                                Qt.KeepAspectRatio,
                                Qt.SmoothTransformation
                            )

                            thumbnail_label = QLabel()
                            thumbnail_label.setPixmap(resized_pixmap)  # Use the resized pixmap for UI
    ...
    def get_label_size(image_path):
        ...
        if os.path.exists(label_path):
            ...
            return -1  # Smaller than any possible label size to push these images to the end
    ...
    def delete_current_image(self):
        ...
        placeholder_path = 'styles/darkfusion.png'
...
# adjust and view labels for a single image
    ...
    def apply_smoke_effect(self, image, intensity=0.1, radius_ratio=1.5, speed=0.1):
        ...
        for scale_factor in np.linspace(0.5, 2.0, num=4):
            smoke_mask = self.generate_smoke_mask(h, w, scale_factor=scale_factor, time_offset=self.time)
        ...
        # Convert combined mask to a 3-channel image
    ...
    def generate_smoke_mask(self, h, w, scale_factor=1.0, time_offset=0.0, octaves=6, persistence=0.5, lacunarity=2.0):
        # Generate Perlin noise values for the entire image
        X, Y = np.meshgrid(np.arange(w), np.arange(h))
        noise = np.vectorize(pnoise2)(
            (X / (20 * scale_factor) + time_offset),
            (Y / (20 * scale_factor)),
            octaves=octaves,
    ...
    def create_mosaic_images(self):
        ...
        while processed_images_count < max_images:
            if not thumbnail_paths:
                # Refill the paths and reshuffle to create new variations
                thumbnail_paths = [
                    os.path.join(thumbnail_dir, f)
                    for f in os.listdir(thumbnail_dir)
                    if os.path.splitext(f)[1].lower() in extensions
            ...
            # Get the current batch of thumbnails
            current_batch = thumbnail_paths[:batch_size]
            thumbnail_paths = thumbnail_paths[batch_size:]
